...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocosts.info/NL',
            text: 'Deze calculator zal u toestaan de werkelijke kost te vinden om een auto te bezitten in Nederland',
            showLabel: false,
            showCount: false,
...
function print_main_table(f1, f2, f3, data) {
    ...
    //header
    varResult+= '<tr><td style="padding:7px;" colspan="4"><b>UW AUTO KOST</b></td></tr>';
    
    varResult+= '<tr>';
    
    varResult+= '<td><b>' + currencyShow(data.total_costs_month.toFixed(0)) + '</b><br>';
    varResult+= 'per<br>maand</td>';
    ...
    varResult+= 'per<br>jaar</td>';
    
    varResult+= '</tr>';
    
    if(f3.IsFinancialEffort){
        varResult+= '<tr><td colspan="4"><b>FINANCIËLE INSPANNING'+
...
function print_result_final_text(data){
    ...
    if(data.total_costs_month >= 150 && data.age_months > 6) {
        ...
        var text_msg = '<div>Met dit niveau van kosten heeft uw auto, tijdens de '+
                       '<b>' + data.age_months + '</b> maanden van bezit, reeds zoveel gekost:</div>'+
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        depreciation_text = "De afschrijving is niet van toepassing omdat dit voertuig nieuw is&nbsp;&nbsp;";
    } else {
        depreciation_text = "<b>Afschrijving van de auto<\/span></b>&nbsp;&nbsp;<br>Aankoopwaarde: "+
            f1.auto_initial_cost + "&euro;<br>Eindwaarde: "+
            f1.auto_final_cost + "&euro;<br>Periode van bezit: "+
            data.age_months + " maanden<br>("+
            f1.auto_initial_cost + "&euro;-"+
            f1.auto_final_cost + "&euro;)/"+
            data.age_months + " maanden";
    }
    ...
    {
        case "semestral":
            insurance_text = f1.insurance_value + " Euro per semester";
            break;
        case "anual":
            insurance_text = f1.insurance_value + " Euro per jaar";
            break;
        case "mensal":
            insurance_text = data.monthly_costs.insurance + " Euro per maand";
            break;
        case "trimestral":
            insurance_text = f1.insurance_value + " Euro per trimester";
            break;
    }
    
    //Credit interests
    var interests_text="<b>Leningrente<\/b>&nbsp;&nbsp;";
    
    if(f1.cred_auto_s_n == "true") {

        interests_text = "<b>Leningrente</b>&nbsp;&nbsp;<br>Gefinancierd bedrag: " +
                         f1.credit_amount +
                         "&euro;<br>Krediettermijn / aantal kredietafleveringen: " +
                         f1.credit_period +
                         " maanden<br>Maandelijkse gemiddelde waarde: " +
                         f1.credit_value_p_month +
                         "&euro;<br>Restwaarde: " +
                         f1.credit_residual_value +
                         "&euro;<br>";    

        interests_text += "Totaal rentebedrag: "+data.total_interests+"&euro;<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Maandelijks rentebedrag: "+data.monthly_costs.credit.toFixed(2)+"&euro;";
        interests_text += "";
    } 
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Autokeuring (Algemene Periodieke Keuring)</b><br>" +
                          f1.nmr_times_inspec +
                          " keren met een kost van " +
                          f1.inspec_price +
                          " &euro; per keer, tijdens " +
                          data.age_months + " maanden&nbsp;";
    }
    else        
        inspection_text = "<b>Autokeuring (Algemene Periodieke Keuring)</b><br>";
    ...
    //Taxes
    var cartax_text = "<b>Motorrijtuigenbelasting (Autobelasting)</b><br>" +
                      f1.car_tax + " Euro per jaar";
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                {
                    case "1":
                        fuel_text = f2.distance + " km per maand";
                        break;
                    case "2":                   
                        fuel_text = f2.distance + " kilometer voor iedere twee maanden";
                        break;
                    case "3":                   
                    ...
                    case "4":                   
                        ...
                        break;
                    case "5":                   
                        fuel_text = f2.distance + " km per jaar";
                        break;
                }
                fuel_text = fuel_text + "<br>" + "Brandstofefficiëntie van uw voertuig: " + f2.car_consumption + " l/100km&nbsp;";
                fuel_text = fuel_text + "<br>" + "Gemiddelde benzineprijs: " + f2.fuel_price + " &euro;/l&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " dag(en) per week die u naar het werk rijdt <br>";
                fuel_text = fuel_text + "U rijdt " + f2.distance_home2job + " kilometers tussen thuis en het werk <br>";
                fuel_text = fuel_text + "U rijdt " + f2.distance_weekend + " gemiddelde kilometer tijdens de dagen die u niet naar het werk rijdt&nbsp;<br>";
                fuel_text = fuel_text + "U rijdt volledig tegen een gemiddelde van " + data.distance_per_month.toFixed(1) + " km per maand (~30.5 dagen) <br>";
                fuel_text = fuel_text + "Brandstofefficiëntie van uw voertuig: " + f2.car_consumption + " l/100km";
                fuel_text = fuel_text + "<br>" + "Gemiddelde prijs die u betaalt voor de benzine: " + f2.fuel_price + " &euro;/l";
            }
            break;
        case "euros":
            ...
            {
                case "1":
                    fuel_text = f2.fuel_money + " Euro per maand";
                    break;
                case "2":                   
                    fuel_text = f2.fuel_money + " kilometer voor iedere twee maanden";
                    break;
                case "3":                   
                    fuel_text = f2.fuel_money + " Euro per trimester";
                    break;
                case "4":                   
                    fuel_text = f2.fuel_money + " Euro per semester";
                    break;
                case "5":                   
                    fuel_text = f2.fuel_money + " Euro per jaar";
                    break;
            }
            break;
    }
    ...
    //Maintenance
    var maintenance_text = "<b>1/2 Onderhoud</b><br>" +
                           f2.maintenance + " Euro per jaar";
    
    //Repairs
    var repairs_text = "<b>Herstellingen en verbeteringen<\/span></b><br>" +
                       f2.repairs + " Euro per jaar";
    
    //Tolls
    var tolls_text="<b>Tol</b><br>";
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                tolls_text += f2.tolls + " Euro per maand";
                break;
            case "2":
                tolls_text += f2.tolls + " Euro bij elke twee maanden";
                break;
            case "3":
                tolls_text += f2.tolls + " Euro per trimester";
                break;
            case "4":
                tolls_text += f2.tolls + " Euro per semester";
                break;
            case "5":
                tolls_text += f2.tolls + " Euro per jaar";
                break;
        }
    }
    else 
        tolls_text+=f2.price_tolls_p_day + " Euro gedurende " + f2.tolls_days_p_month + " maand";
    tolls_text += "";
    
    //Fines
    var fines_text="<b>Verkeersboetes</b><br>";
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Euro per maand";
            break;
        case "2":           
            fines_text += f2.fines + " Euro bij elke twee maanden";
            break;
        case "3":           
            fines_text += f2.fines+" Euro per trimester";
            break;
        case "4":           
            fines_text += f2.fines + " Euro per semester";
            break;
        case "5":           
            fines_text += f2.fines + " Euro per jaar";
            break;
        }
    fines_text+="";
    
    //washing
    var washing_text="<b>Wassen en schoonmaken</b><br>";
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Euro per maand";
            break;
        case "2":
            washing_text += f2.washing + " Euro bij elke twee maanden";
            break;
        case "3":
            washing_text += f2.washing +" Euro per trimester";
            break;
        case "4":
            washing_text += f2.washing + " Euro per semester";
            break;
        case "5":
            washing_text += f2.washing + " Euro per jaar";
            break;
        }
    washing_text+="";
    ...
    varResult+= "<tr><td style=\"padding:10px 50px;\" colspan=\"2\"><b>Vaste kosten</b><br>" +
                "<i>Kosten die niet afhankelijk zijn van de afgelegde afstand en die men ook moet betalen als de auto niet rijdt</i></td></tr>";
    
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Kosten</b></td>" +
                "<td><b>Maandelijks bedrag</b></td></tr>";
    
    //standing costs items
    varResult+= "<tr><td>" + depreciation_text + "&nbsp;</td>" + 
                "<td>&nbsp;" + currencyShow(data.monthly_costs.depreciation.toFixed(1)) + "</td></tr>";
    
    varResult+= "<tr><td><b>Autoverzekering en pechhulpverleningsdienst</b><br>" + insurance_text + "</td>" +
    ...
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>TOTAAL – Vaste kosten</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_standing_costs_month.toFixed(0)) + "/maand</b></td></tr>";
    ...
    varResult+= "<tr><td style=\"padding:10px 15px;\" colspan=\"2\"><b>Exploitatiekosten</b><br>" +
                "<i>Kosten die afhangen van het aantal gereden kilometers</i></td></tr>";
                  
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Kosten</b></td>" +
                "<td><b>Maandelijks bedrag</b></td></tr>";

    varResult+= "<tr><td><b>Brandstof</b><br>" + fuel_text + "</td>" +
    ...
    varResult+= "<tr><td><b>Parkeren</b></td>"+
    ...
    //TOTAL - Running costs
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>TOTAAL - Exploitatiekosten</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_running_costs_month.toFixed(0)) + "/maand</b></td></tr>";
    ...
    varResult+= "<tr><td style=\"padding:10px 15px;\" colspan=\"2\"><b>TOTAAL</b><br></td></tr>"; 

    if((typeof data.distance_per_month) !== 'undefined' && data.distance_per_month !== 0){
              
        varResult+= "<tr><td><b>Exploitatiekosten per kilometer</b></td>"+
                    "<td>&nbsp;" + currencyShow(data.running_costs_p_unit_distance.toFixed(2)) + "/km </td></tr>";
        
        varResult+= "<tr><td class=\"border_bottom_2px\"><b>Totale kosten per kilometer</b></td>" +
                    "<td class=\"border_bottom_2px\">&nbsp;" + currencyShow(data.total_costs_p_unit_distance.toFixed(2)) + "/km </td></tr>";
    }

    varResult+= "<tr><td><b>Vaste kosten</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_standing_costs_month.toFixed(0)) + "/maand</b></td></tr>";
    
    varResult+= "<tr><td><b>Exploitatiekosten</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_running_costs_month.toFixed(0)) + "/maand</b></td></tr>";    
    
    varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAAL</b></td>"+
               "<td>&nbsp;<b>" + currencyShow(data.total_costs_month.toFixed(0)) + "/maand</b></td></tr>";
...
function print_feffort_table(f1, f2, f3, data){
    ...
    varResult+="<tr><td colspan=\"2\"><b>Financiële inspanning</b></td></tr>";
    //income
    varResult+="<tr><td colspan=\"2\"><b>Inkomen</b></tr>";
    switch(f3.income_type){
        case 'year':    
            varResult+= "<tr><td>Netto inkomen per jaar</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Gemiddeld netto inkomen per maand</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>";
            break;
        case 'month':
            varResult+= "<tr><td>Netto inkomen per maand</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Aantal maanden per jaar met inkomen</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Gemiddeld netto inkomen per maand</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>" +
                        "<tr><td>Gemiddeld netto inkomen per jaar</td>" + 
                        "<td>" + currencyShow(data.fin_effort.income_per_year.toFixed(1)) + "</td></tr>";
            break;
        case 'week':
            varResult+= "<tr><td>Netto inkomen per week</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Aantal weken per jaar met inkomen</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Gemiddeld netto inkomen per maand</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>"+
                        "<tr><td>Gemiddeld netto inkomen per jaar</td>" + 
            ...
            break;  
        case 'hour':
            varResult+= "<tr><td>Netto inkomen per uur</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Aantal uren per week met inkomen</td>" + 
    ...
    if(f3.income_type != 'hour'){
        varResult+=     "<tr><td colspan=\"2\"><b>Werktijd</b></tr>";
        if(f3.is_working_time == 'true'){
            varResult+= "<tr><td>Uren per week</td>" +
                        "<td>"+data.fin_effort.time_hours_per_week+" h</td></tr>" +
                        "<tr><td>Maanden per jaar</td>" + 
                        "<td>"+data.fin_effort.time_month_per_year+"</td></tr>" +
                        "<tr><td>Gemiddeld aantal werkuren per maand</td>" + 
                        "<td>"+data.fin_effort.aver_work_time_per_m.toFixed(1)+" h</td></tr>" +
                        "<tr><td>Werkuren per jaar</td>" + 
                        "<td>"+data.fin_effort.work_hours_per_y.toFixed(1)+" h</td></tr>";
        }
        else{
            varResult+= "<tr><td colspan=\"2\">Het was bedoeld voor berekeningen van een gemiddelde werktijd van 36 uur per week en 11 maanden per jaar</td></tr>";
        }
    }           
    varResult+= "<tr><td>Gemiddeld netto inkomen per uur</td>" +
                "<td>&nbsp;" + currencyShow(data.fin_effort.aver_income_per_hour.toFixed(1)) + "</td></tr>";
    
    //distance
    varResult+= "<tr><td colspan=\"2\"><b>Afstand</b></td></tr>";
    if((f2.type_calc_fuel != 'km' && f3.drive_to_work == 'true') || (f2.type_calc_fuel != 'km' && f2.take_car_to_job == 'true')){   
        varResult+=  "<tr><td>U rijdt van uw huis naar uw werk</td>"  +  
                     "<td>" + parseInt(f3.dist_home_job).toFixed(1) + " km</td></tr>"+
                     "<tr><td>Dagen per week die u naar uw werk rijdt</td>"  +  
                     "<td>" + f3.drive_to_work_days_per_week + " dagen</td></tr>" +
                     "<tr><td>U rijdt gedurende de dagen waarop u niet met de auto naar uw werk gaat</td>" + 
                     "<td>" + parseInt(f3.journey_weekend).toFixed(1) + " km</td></tr>"+
                     "<tr><td>U gemiddeld per week rijdt</td>" + 
                     "<td>" + data.driving_distance.aver_drive_per_week.toFixed(1) + " km</td></tr>";                  
    }

    varResult+=  "<tr><td>U rijdt per maand</td>" +
                 "<td>" + data.distance_per_month.toFixed(1) + " km</td></tr>" +
                 "<tr><td>U rijdt per jaar</td>" + 
                 "<td>" + data.driving_distance.drive_per_year.toFixed(1) + " km</td></tr>";  

    //time spent in driving
    varResult+=  "<tr><td colspan=\"2\"><b>Tijd die u besteedt aan rijden</b></td></tr>";

    if(f3.drive_to_work == 'true' || f2.take_car_to_job == 'true'){
        varResult+= "<tr><td>Minuten die u van uw huis naar uw werk rijdt</td>" + 
                    "<td>" + f3.time_home_job + " min</td></tr>" +
                    "<tr><td>Dagen per week die u naar uw werk rijdt</td>" + 
                    "<td>" + f3.drive_to_work_days_per_week + " dagen</td></tr>" +
                    "<tr><td>Minuten die u rijdt op de dagen waarop u niet met de auto naar uw werk gaat</td>" + 
                    "<td>" + f3.time_weekend + " min</td></tr>" +
                    "<tr><td>Minuten die u rijdt per week</td>" + 
                    "<td>" + data.time_spent_driving.min_drive_per_week + " min</td></tr>";
    }
    else{
        varResult+= "<tr><td>Minuten die u rijdt per dag</td>" + 
                    "<td>" + f3.min_drive_per_day + " min</td></tr>" +
                    "<tr><td>Dagen die u rijdt per maand</td>" + 
                    "<td>" + f3.days_drive_per_month + " dagen</td></tr>";
    }

    varResult+= "<tr><td>Uren die u rijdt per maand</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_month.toFixed(1) + " h</td></tr>"+
                "<tr><td>Uren die u rijdt per jaar</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_year.toFixed(1) + " h</td></tr>";

    //financial effort
    varResult+= "<tr><td colspan=\"2\"><b>Financiële inspanning" +
                ': ' + (data.total_costs_year/data.fin_effort.income_per_year*100).toFixed(0) + 
                '&#37;</b>' +
                "<tr><td>Totale kosten per jaar voor uw auto</td>" + 
                "<td>" + currencyShow(data.fin_effort.total_costs_year.toFixed(1)) + "</td></tr>" +
                "<tr><td>Uren per jaar die u moet werken om uw auto te kunnen veroorloven</td>"  +  
                "<td>" + data.fin_effort.hours_per_year_to_afford_car.toFixed(1) + " h</td></tr>"+
                "<tr><td>Maanden per jaar die u moet werken om uw auto te kunnen veroorloven</td>" +  
                "<td>" + data.fin_effort.month_per_year_to_afford_car.toFixed(2)+"</td></tr>"+
                "<tr><td>Voor hoeveel dagen is de auto betaald, vanaf 1 januari</td>" +  
                "<td>" + Math.ceil(data.fin_effort.days_car_paid) + " dagen</td></tr>";
           

    //speed
    varResult+= "<tr><td>Gemiddeld per jaar kinetische snelheid</td>"+
                "<td>" + data.kinetic_speed.toFixed(1) + " km/h</td></tr>";
                        
    varResult+= "<tr><td>Gemiddeld per jaar <a href=\"./docs/consumer_speed.html\" target=\"_blank\">virtuele snelheid</a></td>"+
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        tp_text = "<b>Openbaar vervoer voor uw dagelijks gezinsleven</b><br>Aantal gezinsleden ouder dan 4 jaar: " + 
                  f3.n_pess_familia + " personen" +
                  "<br>Gemiddeld bedrag van het maandelijks abonnement per persoon: " + 
                  f3.monthly_pass_cost + "&euro;";
        
        if(data.public_transports.pt_carcost_ratio < data.public_transports.other_pt_ratio_threshold){
            outros_tp_text="<b>Ander openbaar vervoer</b><br>Bedrag dat nog open stond voor ander openbaar vervoer, bijvoorbeeld vanuit uw woongebied, zoals lange treinreizen of lange busreizen ";
        }
        taxi_text="<b>Taxivervoer<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " km met een betaalde taxi " + data.public_transports.taxi_price_per_km.toFixed(1) + "&euro;/km";
        ...
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAAL</b></td>"+
                   "<td><b>" + currencyShow(data.public_transports.total_altern.toFixed(0)) + "/maand</b></td></tr>";
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>UBER - Kosten per kilometer</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucd.toFixed(2)) + "/" + "km</td></tr>";
                
                varResult+="<tr><td><b>UBER - Kosten per minuten</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucm.toFixed(2)) + "/" + "min</td></tr>";

                varResult+="<tr><td><b>Kilometers die u rijdt per maand</b><br></td>"+
                           "<td>" + res_uber_obj.dpm.toFixed(0) + " " +"kilometer</td></tr>";
                
                           
                varResult+="<tr><td><b>Minuten die u rijdt per maand</b></td>" + 
                           "<td>" + res_uber_obj.mdpm.toFixed(0) + " " + "minuten</td></tr>";
                           
                varResult+="<tr><td><b>UBER: Kosten - TOTAAL</b></td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.tuc.toFixed(0)) + "</b></td></tr>";                     

                varResult+="<tr><td><b>Ander openbaar vervoer</b><br>Bedrag dat nog open stond voor ander openbaar vervoer, bijvoorbeeld vanuit uw woongebied, zoals lange treinreizen of lange busreizen</td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.delta.toFixed(0)) + "</b></td></tr>";
                
                varResult+="<tr><td><b>TOTAAL</b></td>"+
                           "<td><b>" + currencyShow(data.total_costs_month.toFixed(0)) + "/maand</b></td></tr>";
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Openbaar vervoer voor uw dagelijks gezinsleven</b><br>Aantal gezinsleden ouder dan 4 jaar: " + f3.n_pess_familia + " personen" +
                           "<br>Gemiddeld bedrag van het maandelijks abonnement per persoon: " + f3.monthly_pass_cost + "&euro;</td>" +
                ...
                varResult+="<tr><td><b>Kinetische snelheid</b></td>" + 
                           "<td>" + data.kinetic_speed.toFixed(2) + " " +"km/h</td></tr>";
                           
                varResult+="<tr><td><b>UBER - kilometer per maand</b></td>" + 
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        //header
        varResult+="<tr><td><b>Custos externos</b><br>Percorre " +(1 * data.distance_per_month).toFixed(1)+" km/maand</td>" +
                   "<td><b>Maandelijks bedrag</b></td></tr>";
        ...
        varResult+="<tr><td>" + ifr_estr_text + "</td>" + 
         ...
         //total
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAAL</b></td>" +
                   "<td><b>&euro;&nbsp;"+data.external_costs.total_exter().toFixed(0)+"/maand</b></td></tr>";
