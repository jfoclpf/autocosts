...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocostos.info/ES',
            text: 'Esta calculadora te permitirá conocer el coste verdadero de ser propietario de un coche en España',
            showLabel: false,
            showCount: false,
...
function print_main_table(f1, f2, f3, data) {
    ...
    //header
    varResult+= '<tr><td style="padding:7px;" colspan="4"><b>TU COCHE CUESTA</b></td></tr>';
...
function print_result_final_text(data){
    ...
    if(data.total_costs_month >= 150 && data.age_months > 6) {
        ...
        var text_msg = '<div>Con este nivel de gastos, tu vehículo durante '+
                       '<b>' + data.age_months + '</b> meses de posesión han costado ya:</div>'+
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        ...
        depreciation_text = "<b>Depreciación del vehículo<\/span></b>&nbsp;&nbsp;<br>Precio de compra: "+
            f1.auto_initial_cost + "&euro;<br>Precio de hoy: "+
            f1.auto_final_cost + "&euro;<br>Periodo de posesión: "+
    ...
    switch(f1.insurance_type)
    {
        case "semestral":
            ...
            break;
        case "anual":
            insurance_text = f1.insurance_value + " Euros por año";
            break;
        case "mensal":
            insurance_text = data.monthly_costs.insurance + " Euros por mes";
            break;
        case "trimestral":
    ...
    if(f1.cred_auto_s_n == "true") {
        ...
        interests_text = "<b>Intereses del préstamo</b>&nbsp;&nbsp;<br>Cuota financiada: " +
                         f1.credit_amount +
                         "&euro;<br>Periodo de crédito / número de plazos: " +
                         f1.credit_period +
        ...
        interests_text += "Importe total de intereses: "+data.total_interests+"&euro;<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Cuota mensual de intereses: "+data.monthly_costs.credit.toFixed(2)+"&euro;";
        interests_text += "";
    } 
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                fuel_text = fuel_text + "<br>" + "Eficacia del combustible de tu coche: " + f2.car_consumption + " l/100km&nbsp;";
                fuel_text = fuel_text + "<br>" + "Precio medio del combustible: " + f2.fuel_price + " &euro;/L&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " día(s) por semana que vas al trabajo en coche <br>";
                fuel_text = fuel_text + "Conduces " + f2.distance_home2job + " kilómetros entre tu casa y el lugar del trabajo <br>";
                fuel_text = fuel_text + "Conduces " + f2.distance_weekend + " kilómetros promedios los días que no utilizas el coche para ir al trabajar&nbsp;<br>";
                fuel_text = fuel_text + "Conduces una media de " + data.distance_per_month.toFixed(1) + " km por mes (~30.5 días) <br>";
                fuel_text = fuel_text + "Eficacia del combustible de tu coche: " + f2.car_consumption + " l/100km";
                fuel_text = fuel_text + "<br>" + "Gasto promedio por el combustible: " + f2.fuel_price + " &euro;/L";
            }
            break;
        case "euros":
            ...
            {
                case "1":
                    fuel_text = f2.fuel_money + " Euros por mes";
                    break;
                case "2":                   
                ...
                case "4":                   
                    ...
                    break;
                case "5":                   
                    fuel_text = f2.fuel_money + " Euros por año";
                    break;
            }
            break;
    }
    ...
    var maintenance_text = "<b>1/2 Mantenimiento</b><br>" +
                           f2.maintenance + " Euros por año";
    
    //Repairs
    var repairs_text = "<b>Reparaciones y mejoras<\/span></b><br>" +
                       f2.repairs + " Euros por año";
    ...
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                tolls_text += f2.tolls + " Euros por mes";
                break;
            case "2":
                tolls_text += f2.tolls + " Euros por cada dos meses";
                break;
            case "3":
            ...
            case "4":
                ...
                break;
            case "5":
                tolls_text += f2.tolls + " Euros por año";
                break;
        }
    }
    else 
        tolls_text+=f2.price_tolls_p_day + " Euros durante " + f2.tolls_days_p_month + " mes";
    tolls_text += "";
    ...
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Euros por mes";
            break;
        case "2":           
            fines_text += f2.fines + " Euros por cada dos meses";
            break;
        case "3":           
        ...
        case "4":           
            ...
            break;
        case "5":           
            fines_text += f2.fines + " Euros por año";
            break;
        }
    fines_text+="";
    ...
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Euros por mes";
            break;
        case "2":
            washing_text += f2.washing + " Euros por cada dos meses";
            break;
        case "3":
        ...
        case "4":
            ...
            break;
        case "5":
            washing_text += f2.washing + " Euros por año";
            break;
        }
    washing_text+="";
    ...
    varResult+= "<tr><td style=\"padding:10px 50px;\" colspan=\"2\"><b>Gastos fijos</b><br>" +
                "<i>Gastos que no dependen de la distancia recorrida y que tienen que pagarse incluso si el coche está parado</i></td></tr>";
    
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Coste</b></td>" +
    ...
    varResult+= "<tr><td><b>Seguro de vehículo y cobertura de averías</b><br>" + insurance_text + "</td>" +
    ...
    varResult+= "<tr><td><b>Aparcamiento</b></td>"+
    ...
    if((typeof data.distance_per_month) !== 'undefined' && data.distance_per_month !== 0){
        ...
        varResult+= "<tr><td class=\"border_bottom_2px\"><b>Coste total por kilómetro</b></td>" +
...
function print_feffort_table(f1, f2, f3, data){
    ...
    switch(f3.income_type){
        ...
        case 'week':
            ...
            break;  
        case 'hour':
            varResult+= "<tr><td>Ingresos netos hour</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
    ...
    varResult+= "<tr><td>Ingresos medios netos por hour</td>" +
    ...
    if((f2.type_calc_fuel != 'km' && f3.drive_to_work == 'true') || (f2.type_calc_fuel != 'km' && f2.take_car_to_job == 'true')){   
        varResult+=  "<tr><td>Conduces de casa al trabajo</td>"  +  
                     ...
                     "<tr><td>La distancia que recorres los días que no vas en coche a trabajar</td>" + 
                     "<td>" + parseInt(f3.journey_weekend).toFixed(1) + " km</td></tr>"+
                     "<tr><td>Conduces semanalmente un promedio de</td>" + 
    ...
    if(f3.drive_to_work == 'true' || f2.take_car_to_job == 'true'){
        varResult+= "<tr><td>Número de minutos que conduces para llegar de casa al trabajo</td>" + 
                    "<td>" + f3.time_home_job + " min</td></tr>" +
                    "<tr><td>Número de días por semana que vas en coche al trabajo</td>" + 
                    "<td>" + f3.drive_to_work_days_per_week + " días</td></tr>" +
                    "<tr><td>Número de minutos empleados en conducción los días que no vas en coche al trabajo</td>" + 
                    "<td>" + f3.time_weekend + " min</td></tr>" +
                    "<tr><td>Número de minutos que conduces por semana</td>" + 
                    "<td>" + data.time_spent_driving.min_drive_per_week + " min</td></tr>";
    }
    else{
        varResult+= "<tr><td>Número de minutos que conduces por día</td>" + 
                    "<td>" + f3.min_drive_per_day + " min</td></tr>" +
                    "<tr><td>Número de días mensuales que conduces</td>" + 
    ...
    varResult+= "<tr><td colspan=\"2\"><b>Esfuerzo financiero" +
                ...
                "<tr><td>Número de horas al año que debes trabajar para pagar por el coche</td>"  +  
                "<td>" + data.fin_effort.hours_per_year_to_afford_car.toFixed(1) + " h</td></tr>"+
                "<tr><td>Número de meses al año que debes trabajar para pagar por el coche</td>" +  
                "<td>" + data.fin_effort.month_per_year_to_afford_car.toFixed(2)+"</td></tr>"+
                "<tr><td>¿Por cuántos días, después del 1 de enero, el coche ya está pagado?</td>" +  
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        tp_text = "<b>Transporte público en la vida diaria de su familia</b><br>Número de miembros de tu familia mayores de 4 años: " + 
        ...
        if(data.public_transports.pt_carcost_ratio < data.public_transports.other_pt_ratio_threshold){
            outros_tp_text="<b>Otros transportes públicos</b><br>Cuota que pagaste por otro tipo de transporte público fuera de tu zona, como para los viajes en trenes y buses de largo recorrido ";
        }
        taxi_text="<b>Transporte en taxi<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " km en taxi pagando " + data.public_transports.taxi_price_per_km.toFixed(1) + "&euro;/km";
        
        //starts HTML table
        varResult+="<table class=\"result_table\" id=\"result_table2\">";
        //header
        varResult+="<tr><td><b>Coste</b><br></td>"+
                   "<td><b>Cuota mensual</b></td></tr>";
        //items
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>UBER - Coste por kilómetros</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucd.toFixed(2)) + "/" + "km</td></tr>";
                
                varResult+="<tr><td><b>UBER - Coste por minutes</b>" + uber_url_HTML + "</td>" + 
                ...
                varResult+="<tr><td><b>Número de minutos que conduces por mes</b></td>" + 
                           "<td>" + res_uber_obj.mdpm.toFixed(0) + " " + "minutes</td></tr>";
                           
                varResult+="<tr><td><b>UBER: Coste - TOTAL</b></td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.tuc.toFixed(0)) + "</b></td></tr>";                     

                varResult+="<tr><td><b>Otros transportes públicos</b><br>Cuota que pagaste por otro tipo de transporte público fuera de tu zona, como para los viajes en trenes y buses de largo recorrido</td>" + 
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Transporte público en la vida diaria de su familia</b><br>Número de miembros de tu familia mayores de 4 años: " + f3.n_pess_familia + " persona(s)" +
                           "<br>Promedio de gastos en el abono mensual por persona: " + f3.monthly_pass_cost + "&euro;</td>" +
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        varResult+="<tr><td>" + ifr_estr_text + "</td>" + 
         ...
         //total
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAL</b></td>" +
                   "<td><b>&euro;&nbsp;"+data.external_costs.total_exter().toFixed(0)+"/mes</b></td></tr>";
