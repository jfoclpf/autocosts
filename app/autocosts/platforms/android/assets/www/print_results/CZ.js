...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocosts.info/CZ',
            text: 'Tento kalkulátor vám umožní zjistit skutečné náklady na vlastnictví auta v České republice',
            showLabel: false,
            showCount: false,
...
function print_result_final_text(data){
    ...
    if(data.total_costs_month >= 150 && data.age_months > 6) {
        ...
        var text_msg = '<div>S touto úrovní nákladů, vás vaše vozidlo během '+
                       '<b>' + data.age_months + '</b> mesíců držení už stálo:</div>'+
                       '<div class="red_bold_text">'+
                       numberWithSpaces((data.age_months * data.total_costs_month / 100).toFixed(0)*100)+
                       ' ' + 'KORUNY ČESKÈ</div></div>';
        return text_msg;
    }
    else{
        return "";
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        depreciation_text = "Opotřebení se neuvádi, protože toto vozidlo je nové&nbsp;&nbsp;";
    } else {
        depreciation_text = "<b>Opotřebení vozidla<\/span></b>&nbsp;&nbsp;<br>Pořizovací hodnota: "+
            f1.auto_initial_cost + "Kč<br>Konečná hodnota: "+
            f1.auto_final_cost + "Kč<br>Doba držení vozidla: "+
            data.age_months + " měsíců<br>("+
            f1.auto_initial_cost + "Kč-"+
            f1.auto_final_cost + "Kč)/"+
            data.age_months + " měsíců";
    }
    ...
    {
        case "semestral":
            insurance_text = f1.insurance_value + " Koruny české za půlrok";
            break;
        case "anual":
            insurance_text = f1.insurance_value + " Koruny české za rok";
            break;
        case "mensal":
            insurance_text = data.monthly_costs.insurance + " Koruny české za měsíc";
            break;
        case "trimestral":
            insurance_text = f1.insurance_value + " Koruny české za čtvrtrok";
            break;
    }
    
    //Credit interests
    var interests_text="<b>Úroky z půjček<\/b>&nbsp;&nbsp;";
    
    if(f1.cred_auto_s_n == "true") {

        interests_text = "<b>Úroky z půjček</b>&nbsp;&nbsp;<br>Financovaná částka: " +
                         f1.credit_amount +
                         "Kč<br>Úvěrové období/počet splátek úvěru: " +
                         f1.credit_period +
                         " měsíců<br>Průměrná měsíční hodnota: " +
                         f1.credit_value_p_month +
                         "Kč<br>Zůstatková hodnota: " +
                         f1.credit_residual_value +
                         "Kč<br>";    

        interests_text += "Celková suma úroků: "+data.total_interests+"Kč<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Měsíční suma úroků: "+data.monthly_costs.credit.toFixed(2)+"Kč";
        interests_text += "";
    } 
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Prohlídka vozidla (Technická prohlídka STK)</b><br>" +
                          f1.nmr_times_inspec +
                          " krát vyúčtovano " +
                          f1.inspec_price +
                          " Kč každá z nich v průběhu " +
                          data.age_months + " měsíců&nbsp;";
    }
    else        
        inspection_text = "<b>Prohlídka vozidla (Technická prohlídka STK)</b><br>";
    ...
    //Taxes
    var cartax_text = "<b>Silniční daň (Daň z motorových vozidel)</b><br>" +
                      f1.car_tax + " Koruny české za rok";
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                {
                    case "1":
                        fuel_text = f2.distance + " km za měsíc";
                        break;
                    case "2":                   
                        fuel_text = f2.distance + " kilometrů za každé dva měsíce";
                        break;
                    case "3":                   
                        fuel_text = f2.distance + " km za čtvrtrok";
                        break;
                    case "4":                   
                        fuel_text = f2.distance + " km za půlrok";
                        break;
                    case "5":                   
                        fuel_text = f2.distance + " km za rok";
                        break;
                }
                fuel_text = fuel_text + "<br>" + "Spotřeba paliva vášho vozidla: " + f2.car_consumption + " l/100km&nbsp;";
                fuel_text = fuel_text + "<br>" + "Průměrná cena paliva: " + f2.fuel_price + " Kč/l&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " Den(y) za týden, které jezdíte do práce <br>";
                fuel_text = fuel_text + "Najedzíte " + f2.distance_home2job + " kilometrů mezi domovem a místem vaší práce <br>";
                fuel_text = fuel_text + "Najedzíte " + f2.distance_weekend + " kilometry, které průměrně najezdíte za ty dny, kdy nejezdíte autem do práce&nbsp;<br>";
                fuel_text = fuel_text + "Celkově najezdíte v průměru " + data.distance_per_month.toFixed(1) + " km za měsíc (~30.5 dny) <br>";
                fuel_text = fuel_text + "Spotřeba paliva vášho vozidla: " + f2.car_consumption + " l/100km";
                fuel_text = fuel_text + "<br>" + "Průměrná cena, kterou zaplatíte za palivo: " + f2.fuel_price + " Kč/l";
            }
            break;
        case "euros":
            ...
            {
                case "1":
                    fuel_text = f2.fuel_money + " Koruny české za měsíc";
                    break;
                case "2":                   
                    fuel_text = f2.fuel_money + " kilometrů za každé dva měsíce";
                    break;
                case "3":                   
                    fuel_text = f2.fuel_money + " Koruny české za čtvrtrok";
                    break;
                case "4":                   
                    fuel_text = f2.fuel_money + " Koruny české za půlrok";
                    break;
                case "5":                   
                    fuel_text = f2.fuel_money + " Koruny české za rok";
                    break;
            }
            break;
    }
    ...
    //Maintenance
    var maintenance_text = "<b>1/2 Údržba</b><br>" +
                           f2.maintenance + " Koruny české za rok";
    
    //Repairs
    var repairs_text = "<b>Opravy a vylepšení<\/span></b><br>" +
                       f2.repairs + " Koruny české za rok";
    
    //Tolls
    var tolls_text="<b>Mýtné poplatky</b><br>";
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                tolls_text += f2.tolls + " Koruny české za měsíc";
                break;
            case "2":
                tolls_text += f2.tolls + " Koruny české za každý dva měsíce";
                break;
            case "3":
                tolls_text += f2.tolls + " Koruny české za čtvrtrok";
                break;
            case "4":
                tolls_text += f2.tolls + " Koruny české za půlrok";
                break;
            case "5":
                tolls_text += f2.tolls + " Koruny české za rok";
                break;
        }
    }
    else 
        tolls_text+=f2.price_tolls_p_day + " Koruny české během " + f2.tolls_days_p_month + " měsíc";
    tolls_text += "";
    
    //Fines
    var fines_text="<b>Pokuty za dopravní přestupky</b><br>";
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Koruny české za měsíc";
            break;
        case "2":           
            fines_text += f2.fines + " Koruny české za každý dva měsíce";
            break;
        case "3":           
            fines_text += f2.fines+" Koruny české za čtvrtrok";
            break;
        case "4":           
            fines_text += f2.fines + " Koruny české za půlrok";
            break;
        case "5":           
            fines_text += f2.fines + " Koruny české za rok";
            break;
        }
    fines_text+="";
    
    //washing
    var washing_text="<b>Mytí a čištění</b><br>";
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Koruny české za měsíc";
            break;
        case "2":
            washing_text += f2.washing + " Koruny české za každý dva měsíce";
            break;
        case "3":
            washing_text += f2.washing +" Koruny české za čtvrtrok";
            break;
        case "4":
            washing_text += f2.washing + " Koruny české za půlrok";
            break;
        case "5":
            washing_text += f2.washing + " Koruny české za rok";
            break;
        }
    washing_text+="";
    ...
    varResult+= "<tr><td style=\"padding:10px 50px;\" colspan=\"2\"><b>Fixní náklady</b><br>" +
                "<i>Náklady, které jsou nezávislé na počtu ujetých kilometrů. Jsou nezbytné pro udržení vozidla ve stavu připravenosti</i></td></tr>";
    
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Náklady</b></td>" +
                "<td><b>Měsíční částka</b></td></tr>";
    
    //standing costs items
    varResult+= "<tr><td>" + depreciation_text + "&nbsp;</td>" + 
                "<td>&nbsp;" + currencyShow(data.monthly_costs.depreciation.toFixed(1)) + "</td></tr>";
    
    varResult+= "<tr><td><b>Povinné ručení a havarijní pojištění</b><br>" + insurance_text + "</td>" +
    ...
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>CELKEM - Fixní náklady</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_standing_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";
    ...
    varResult+= "<tr><td style=\"padding:10px 15px;\" colspan=\"2\"><b>Provozní náklady</b><br>" +
                "<i>Náklady, které jsou závislé od počtu naježděných kilometrů</i></td></tr>";
                  
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Náklady</b></td>" +
                "<td><b>Měsíční částka</b></td></tr>";

    varResult+= "<tr><td><b>Palivo</b><br>" + fuel_text + "</td>" +
    ...
    varResult+= "<tr><td><b>Parkování</b></td>"+
    ...
    //TOTAL - Running costs
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>CELKEM - Provozní náklady</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_running_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";
    ...
    varResult+= "<tr><td style=\"padding:10px 15px;\" colspan=\"2\"><b>CELKEM</b><br></td></tr>"; 

    if((typeof data.distance_per_month) !== 'undefined' && data.distance_per_month !== 0){
              
        varResult+= "<tr><td><b>Provozní náklady na kilometr</b></td>"+
                    "<td>&nbsp;" + currencyShow(data.running_costs_p_unit_distance.toFixed(2)) + "/km </td></tr>";
        
        varResult+= "<tr><td class=\"border_bottom_2px\"><b>Celkové náklady na kilometr</b></td>" +
                    "<td class=\"border_bottom_2px\">&nbsp;" + currencyShow(data.total_costs_p_unit_distance.toFixed(2)) + "/km </td></tr>";
    }

    varResult+= "<tr><td><b>Fixní náklady</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_standing_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";
    
    varResult+= "<tr><td><b>Provozní náklady</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_running_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";    
    
    varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>CELKEM</b></td>"+
               "<td>&nbsp;<b>" + currencyShow(data.total_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";
...
function print_feffort_table(f1, f2, f3, data){
    ...
    varResult+="<tr><td colspan=\"2\"><b>Finanční úsilí</b></td></tr>";
    //income
    varResult+="<tr><td colspan=\"2\"><b>Příjem</b></tr>";
    switch(f3.income_type){
        case 'year':    
            varResult+= "<tr><td>Čistý příjem za rok</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Průměrný čistý příjem za měsíc</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>";
            break;
        case 'month':
            varResult+= "<tr><td>Čistý příjem za měsíc</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Počet měsíců ročního příjmu</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Průměrný čistý příjem za měsíc</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>" +
                        "<tr><td>Průměrný čistý příjem za rok</td>" + 
                        "<td>" + currencyShow(data.fin_effort.income_per_year.toFixed(1)) + "</td></tr>";
            break;
        case 'week':
            varResult+= "<tr><td>Čistý příjem za týden</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Počet týdnů ročního příjmu</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Průměrný čistý příjem za měsíc</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>"+
                        "<tr><td>Průměrný čistý příjem za rok</td>" + 
            ...
            break;  
        case 'hour':
            varResult+= "<tr><td>Čistý příjem za hodina</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Počet hodin týdenního příjmu</td>" + 
    ...
    if(f3.income_type != 'hour'){
        varResult+=     "<tr><td colspan=\"2\"><b>Pracovní doba</b></tr>";
        if(f3.is_working_time == 'true'){
            varResult+= "<tr><td>Hodin za týden</td>" +
                        "<td>"+data.fin_effort.time_hours_per_week+" h</td></tr>" +
                        "<tr><td>Měsíců za rok</td>" + 
                        "<td>"+data.fin_effort.time_month_per_year+"</td></tr>" +
                        "<tr><td>Průměrná pracovní doba za měsíc</td>" + 
                        "<td>"+data.fin_effort.aver_work_time_per_m.toFixed(1)+" h</td></tr>" +
                        "<tr><td>Pracovní doba za rok</td>" + 
                        "<td>"+data.fin_effort.work_hours_per_y.toFixed(1)+" h</td></tr>";
        }
        else{
            varResult+= "<tr><td colspan=\"2\">Při výpočtu se uvažovalo s průměrným pracovním časem 36 hodin týdně a 11 měsíců ročně</td></tr>";
        }
    }           
    varResult+= "<tr><td>Průměrný čistý příjem za hodina</td>" +
                "<td>&nbsp;" + currencyShow(data.fin_effort.aver_income_per_hour.toFixed(1)) + "</td></tr>";
    
    //distance
    varResult+= "<tr><td colspan=\"2\"><b>Vzdálenost</b></td></tr>";
    if((f2.type_calc_fuel != 'km' && f3.drive_to_work == 'true') || (f2.type_calc_fuel != 'km' && f2.take_car_to_job == 'true')){   
        varResult+=  "<tr><td>Jezdíte z domu do práce</td>"  +  
                     "<td>" + parseInt(f3.dist_home_job).toFixed(1) + " km</td></tr>"+
                     "<tr><td>Dní v týdnu jezdíte do práce</td>"  +  
                     "<td>" + f3.drive_to_work_days_per_week + " dny</td></tr>" +
                     "<tr><td>Najezdíte během dní, kdy na pracoviště nejezdíte autem?</td>" + 
                     "<td>" + parseInt(f3.journey_weekend).toFixed(1) + " km</td></tr>"+
                     "<tr><td>Najezdíte v průměru za týden</td>" + 
                     "<td>" + data.driving_distance.aver_drive_per_week.toFixed(1) + " km</td></tr>";                  
    }

    varResult+=  "<tr><td>Najezdíte za měsíc</td>" +
                 "<td>" + data.distance_per_month.toFixed(1) + " km</td></tr>" +
                 "<tr><td>Najezdíte za rok</td>" + 
                 "<td>" + data.driving_distance.drive_per_year.toFixed(1) + " km</td></tr>";  

    //time spent in driving
    varResult+=  "<tr><td colspan=\"2\"><b>Čas strávený řízením</b></td></tr>";

    if(f3.drive_to_work == 'true' || f2.take_car_to_job == 'true'){
        varResult+= "<tr><td>Minut jezdíte z domu na pracoviště</td>" + 
                    "<td>" + f3.time_home_job + " min</td></tr>" +
                    "<tr><td>Dní v týdnu jezdíte do práce</td>" + 
                    "<td>" + f3.drive_to_work_days_per_week + " dny</td></tr>" +
                    "<tr><td>Minut najezdíte ve dnech, kdy na pracoviště nejezdíte autem?</td>" + 
                    "<td>" + f3.time_weekend + " min</td></tr>" +
                    "<tr><td>Minut najezdíte za týden</td>" + 
                    "<td>" + data.time_spent_driving.min_drive_per_week + " min</td></tr>";
    }
    else{
        varResult+= "<tr><td>Minut najezdíte za den</td>" + 
                    "<td>" + f3.min_drive_per_day + " min</td></tr>" +
                    "<tr><td>Dní najezdíte za měsíc</td>" + 
                    "<td>" + f3.days_drive_per_month + " dny</td></tr>";
    }

    varResult+= "<tr><td>Hodin najezdíte za měsíc</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_month.toFixed(1) + " h</td></tr>"+
                "<tr><td>Hodin najezdíte za rok</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_year.toFixed(1) + " h</td></tr>";

    //financial effort
    varResult+= "<tr><td colspan=\"2\"><b>Finanční úsilí" +
                ': ' + (data.total_costs_year/data.fin_effort.income_per_year*100).toFixed(0) + 
                '&#37;</b>' +
                "<tr><td>Celkové roční náklady na auto</td>" + 
                "<td>" + currencyShow(data.fin_effort.total_costs_year.toFixed(1)) + "</td></tr>" +
                "<tr><td>Hodin ročně je třeba odpracovat, aby jste si mohli dovolit auto</td>"  +  
                "<td>" + data.fin_effort.hours_per_year_to_afford_car.toFixed(1) + " h</td></tr>"+
                "<tr><td>Měsíců ročně je třeba odpracovat, aby jste si mohli dovolit auto</td>" +  
                "<td>" + data.fin_effort.month_per_year_to_afford_car.toFixed(2)+"</td></tr>"+
                "<tr><td>Na kolik dní po 1. lednu je auto zaplaceno</td>" +  
                "<td>" + Math.ceil(data.fin_effort.days_car_paid) + " dny</td></tr>";
           

    //speed
    varResult+= "<tr><td>Průměrná roční kinetická rychlost</td>"+
                "<td>" + data.kinetic_speed.toFixed(1) + " km/h</td></tr>";
                        
    varResult+= "<tr><td>Průměrná roční <a href=\"./docs/consumer_speed.html\" target=\"_blank\">skutečná rychlost</a></td>"+
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        tp_text = "<b>Veřejná doprava pro každodenní život členů vaší rodiny</b><br>Počet členů vaší rodiny starších 4 let: " + 
                  f3.n_pess_familia + " osoba(y)" +
                  "<br>Průměrná cena měsíční jízdenky pro jednu osobu: " + 
                  f3.monthly_pass_cost + "Kč";
        
        if(data.public_transports.pt_carcost_ratio < data.public_transports.other_pt_ratio_threshold){
            outros_tp_text="<b>Jiná veřejní doprava</b><br>Částka, která ještě zůstala na jiné druhy veřejné dopravy, například mimo vaší obytní zónu, takových jako cestováni vlakem nebo dálkovým autobusem ";
        }
        taxi_text="<b>Taxi doprava<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " km zaplacené za taxislužbu " + data.public_transports.taxi_price_per_km.toFixed(1) + "Kč/km";
        
        //starts HTML table
        varResult+="<table class=\"result_table\" id=\"result_table2\">";
        //header
        varResult+="<tr><td><b>Náklady</b><br></td>"+
                   "<td><b>Měsíční částka</b></td></tr>";
        //items
        ...
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>CELKEM</b></td>"+
                   "<td><b>" + currencyShow(data.public_transports.total_altern.toFixed(0)) + "/měsíc</b></td></tr>";
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>UBER - Náklady za kilometr</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucd.toFixed(2)) + "/" + "km</td></tr>";
                
                varResult+="<tr><td><b>UBER - Náklady za minut</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucm.toFixed(2)) + "/" + "min</td></tr>";

                varResult+="<tr><td><b>Kilometry, které najezdíte za měsíc</b><br></td>"+
                           "<td>" + res_uber_obj.dpm.toFixed(0) + " " +"kilometr</td></tr>";
                
                           
                varResult+="<tr><td><b>Minut najezdíte za měsíc</b></td>" + 
                           "<td>" + res_uber_obj.mdpm.toFixed(0) + " " + "minut</td></tr>";
                           
                varResult+="<tr><td><b>UBER: Náklady - CELKEM</b></td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.tuc.toFixed(0)) + "</b></td></tr>";                     

                varResult+="<tr><td><b>Jiná veřejní doprava</b><br>Částka, která ještě zůstala na jiné druhy veřejné dopravy, například mimo vaší obytní zónu, takových jako cestováni vlakem nebo dálkovým autobusem</td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.delta.toFixed(0)) + "</b></td></tr>";
                
                varResult+="<tr><td><b>CELKEM</b></td>"+
                           "<td><b>" + currencyShow(data.total_costs_month.toFixed(0)) + "/měsíc</b></td></tr>";
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Veřejná doprava pro každodenní život členů vaší rodiny</b><br>Počet členů vaší rodiny starších 4 let: " + f3.n_pess_familia + " osoba(y)" +
                           "<br>Průměrná cena měsíční jízdenky pro jednu osobu: " + f3.monthly_pass_cost + "Kč</td>" +
                ...
                varResult+="<tr><td><b>Kinetická rychlost</b></td>" + 
                           "<td>" + data.kinetic_speed.toFixed(2) + " " +"km/h</td></tr>";
                           
                varResult+="<tr><td><b>UBER - kilometr za měsíc</b></td>" + 
                           "<td>" + res_uber_obj.dist_uber.toFixed(0) + " " + "kilometr</td></tr>";
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    var epa_text      = "<b>Emissões de poluentes atmosféricos</b><br>Valor aproximado: " + data.external_costs.polution + "Kč/km";
    var egee_text     = "<b>Emissões de gases de efeito de estufa</b><br>Valor aproximado: " + data.external_costs.ghg + "Kč/km";
    var ruido_text    = "<b>Poluição sonora</b><br>Valor aproximado: " + data.external_costs.noise + "Kč/km";
    var sr_text       = "<b>Sinistralidade rodoviária</b><br>Valor aproximado: " + data.external_costs.fatalities + "Kč/km";
    var cgstn_text    = "<b>Congestionamento<\/span></b><br>Valor aproximado: " + data.external_costs.congestion + "Kč/km";
    var ifr_estr_text = "<b>Desgaste das infraestruturas rodoviárias</b><br>Valor aproximado: " + data.external_costs.infrastr + "Kč/km";
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        //header
        varResult+="<tr><td><b>Custos externos</b><br>Percorre " +(1 * data.distance_per_month).toFixed(1)+" km/měsíc</td>" +
                   "<td><b>Měsíční částka</b></td></tr>";
        
        //external costs items
        varResult+="<tr><td>" + epa_text + "</td>" +   
                   "<td>&nbsp;Kč " + (data.external_costs.polution * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + egee_text + "</td>" + 
                   "<td>&nbsp;Kč " + (data.external_costs.ghg * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ruido_text + "</td>" + 
                   "<td>&nbsp;Kč " + (data.external_costs.noise * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + sr_text + "</td>" + 
                   "<td>&nbsp;Kč " + (data.external_costs.fatalities * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + cgstn_text + "</td>" + 
                   "<td>&nbsp;Kč " + (data.external_costs.congestion * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ifr_estr_text + "</td>" + 
                   "<td>&nbsp;Kč " + (data.external_costs.infrastr * data.distance_per_month).toFixed(1)+"</td></tr>";
        
         //total
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>CELKEM</b></td>" +
                   "<td><b>Kč&nbsp;"+data.external_costs.total_exter().toFixed(0)+"/měsíc</b></td></tr>";
