...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocustos.info/BR',
            text: 'Faça com essa calculadora as contas, ao verdadeiro custo que tem seu veículo particular no Brasil',
            showLabel: false,
            showCount: false,
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        depreciation_text = "Não se aplica a depreciação do veículo pois este é novo&nbsp;&nbsp;";
    } else {
        depreciation_text = "<b>Depreciação do veículo<\/span></b>&nbsp;&nbsp;<br>Valor de aquisição: "+
            f1.auto_initial_cost + "R$<br>Valor hoje: "+
            f1.auto_final_cost + "R$<br>Período de posse: "+
            data.age_months + " meses<br>("+
            f1.auto_initial_cost + "R$-"+
            f1.auto_final_cost + "R$)/"+
    ...
    {
        case "semestral":
            insurance_text = f1.insurance_value + " Reais por semestre";
            break;
        case "anual":
            insurance_text = f1.insurance_value + " Reais por ano";
            break;
        case "mensal":
            insurance_text = data.monthly_costs.insurance + " Reais por mês";
            break;
        case "trimestral":
            insurance_text = f1.insurance_value + " Reais por trimestre";
            break;
    }
    ...
    if(f1.cred_auto_s_n == "true") {
        ...
        interests_text = "<b>Juros do crédito</b>&nbsp;&nbsp;<br>Montante financiado: " +
                         f1.credit_amount +
                         "R$<br>Período do crédito / n.º de prestações: " +
                         f1.credit_period +
        ...
        interests_text += "Valor total de juros: "+data.total_interests+"R$<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Valor mensal de juros: "+data.monthly_costs.credit.toFixed(2)+"R$";
        interests_text += "";
    } 
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Vistoria automóvel</b><br>" +
                          f1.nmr_times_inspec +
                          " vezes custando " +
                          f1.inspec_price +
                          " R$ cada uma durante " +
    ...
    //Taxes
    var cartax_text = "<b>IPVA, DPVAT e taxa de licenciamento</b><br>" +
                      f1.car_tax + " Reais por ano";
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                fuel_text = fuel_text + "<br>" + "Consumo médio do seu veículo: " + f2.car_consumption + " km/l&nbsp;";
                fuel_text = fuel_text + "<br>" + "Preço médio do combustível: " + f2.fuel_price + " R$/ltr&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " dia(s) por semana que leva o carro para o emprego <br>";
                fuel_text = fuel_text + "Faz " + f2.distance_home2job + " Quilômetros entre casa e o emprego <br>";
                fuel_text = fuel_text + "Faz " + f2.distance_weekend + " Quilômetros por semana nos dias que não leva o carro para o emprego&nbsp;<br>";
                fuel_text = fuel_text + "No total faz em média " + data.distance_per_month.toFixed(1) + " km por mês (~30.5 dias) <br>";
                fuel_text = fuel_text + "Consumo médio do seu veículo: " + f2.car_consumption + " km/l";
                fuel_text = fuel_text + "<br>" + "Preço médio que paga pelo combustível: " + f2.fuel_price + " R$/ltr";
            }
            break;
        case "euros":
            ...
            {
                case "1":
                    fuel_text = f2.fuel_money + " Reais por mês";
                    break;
                case "2":                   
                    fuel_text = f2.fuel_money + " km por cada dois meses";
                    break;
                case "3":                   
                    fuel_text = f2.fuel_money + " Reais por trimestre";
                    break;
                case "4":                   
                    fuel_text = f2.fuel_money + " Reais por semestre";
                    break;
                case "5":                   
                    fuel_text = f2.fuel_money + " Reais por ano";
                    break;
            }
            break;
    }
    ...
    //Maintenance
    var maintenance_text = "<b>1/2 Manutenção preventiva</b><br>" +
                           f2.maintenance + " Reais por ano";
    
    //Repairs
    var repairs_text = "<b>Reparações e Melhoramentos<\/span></b><br>" +
                       f2.repairs + " Reais por ano";
    
    //Tolls
    var tolls_text="<b>Pedágios</b><br>";
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                tolls_text += f2.tolls + " Reais por mês";
                break;
            case "2":
                tolls_text += f2.tolls + " Reais por cada dois meses";
                break;
            case "3":
                tolls_text += f2.tolls + " Reais por trimestre";
                break;
            case "4":
                tolls_text += f2.tolls + " Reais por semestre";
                break;
            case "5":
                tolls_text += f2.tolls + " Reais por ano";
                break;
        }
    }
    else 
        tolls_text+=f2.price_tolls_p_day + " Reais durante " + f2.tolls_days_p_month + " mês";
    tolls_text += "";
    ...
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Reais por mês";
            break;
        case "2":           
            fines_text += f2.fines + " Reais por cada dois meses";
            break;
        case "3":           
            fines_text += f2.fines+" Reais por trimestre";
            break;
        case "4":           
            fines_text += f2.fines + " Reais por semestre";
            break;
        case "5":           
            fines_text += f2.fines + " Reais por ano";
            break;
        }
    fines_text+="";
    
    //washing
    var washing_text="<b>Lavagens e limpeza</b><br>";
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Reais por mês";
            break;
        case "2":
            washing_text += f2.washing + " Reais por cada dois meses";
            break;
        case "3":
            washing_text += f2.washing +" Reais por trimestre";
            break;
        case "4":
            washing_text += f2.washing + " Reais por semestre";
            break;
        case "5":
            washing_text += f2.washing + " Reais por ano";
            break;
        }
    washing_text+="";
    ...
    varResult+= "<tr><td><b>Seguro automóvel (excluindo DPVAT)</b><br>" + insurance_text + "</td>" +
    ...
    varResult+= "<tr><td><b>Estacionamento</b></td>"+
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        tp_text = "<b>Transportes públicos na sua área de residência</b><br>Agregado familiar, de pessoas com mais de 4 anos de idade: " + 
                  ...
                  "<br>Valor pago em transportes públicos para viagens regulares: " + 
                  f3.monthly_pass_cost + "R$";
        ...
        taxi_text="<b>Deslocações de táxi<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " km de táxi a " + data.public_transports.taxi_price_per_km.toFixed(1) + "R$/km";
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>UBER - Custos por quilômetros</b>" + uber_url_HTML + "</td>" + 
                ...
                varResult+="<tr><td><b>Quilômetros que percorre por mês</b><br></td>"+
                           "<td>" + res_uber_obj.dpm.toFixed(0) + " " +"quilômetros</td></tr>";
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Transportes públicos na sua área de residência</b><br>Agregado familiar, de pessoas com mais de 4 anos de idade: " + f3.n_pess_familia + " pessoa(s)" +
                           "<br>Valor pago em transportes públicos para viagens regulares: " + f3.monthly_pass_cost + "R$</td>" +
                ...
                varResult+="<tr><td><b>UBER - quilômetros por mês</b></td>" + 
                           "<td>" + res_uber_obj.dist_uber.toFixed(0) + " " + "quilômetros</td></tr>";
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    var epa_text      = "<b>Emissões de poluentes atmosféricos</b><br>Valor aproximado: " + data.external_costs.polution + "R$/km";
    var egee_text     = "<b>Emissões de gases de efeito de estufa</b><br>Valor aproximado: " + data.external_costs.ghg + "R$/km";
    var ruido_text    = "<b>Poluição sonora</b><br>Valor aproximado: " + data.external_costs.noise + "R$/km";
    var sr_text       = "<b>Sinistralidade rodoviária</b><br>Valor aproximado: " + data.external_costs.fatalities + "R$/km";
    var cgstn_text    = "<b>Congestionamento<\/span></b><br>Valor aproximado: " + data.external_costs.congestion + "R$/km";
    var ifr_estr_text = "<b>Desgaste das infraestruturas rodoviárias</b><br>Valor aproximado: " + data.external_costs.infrastr + "R$/km";
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        varResult+="<tr><td>" + epa_text + "</td>" +   
                   "<td>&nbsp;R$ " + (data.external_costs.polution * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + egee_text + "</td>" + 
                   "<td>&nbsp;R$ " + (data.external_costs.ghg * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ruido_text + "</td>" + 
                   "<td>&nbsp;R$ " + (data.external_costs.noise * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + sr_text + "</td>" + 
                   "<td>&nbsp;R$ " + (data.external_costs.fatalities * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + cgstn_text + "</td>" + 
                   "<td>&nbsp;R$ " + (data.external_costs.congestion * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ifr_estr_text + "</td>" + 
                   "<td>&nbsp;R$ " + (data.external_costs.infrastr * data.distance_per_month).toFixed(1)+"</td></tr>";
        
         //total
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAL</b></td>" +
                   "<td><b>R$&nbsp;"+data.external_costs.total_exter().toFixed(0)+"/mês</b></td></tr>";
