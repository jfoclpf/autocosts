...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocosts.info/UK',
            text: 'This calculator will allow you to find the true cost of owning a car in the United Kingdom',
            showLabel: false,
            showCount: false,
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        ...
        depreciation_text = "<b>Depreciation of the vehicle<\/span></b>&nbsp;&nbsp;<br>Acquisition value: "+
            f1.auto_initial_cost + "&pound;<br>Value today: "+
            f1.auto_final_cost + "&pound;<br>Period of possession: "+
            data.age_months + " months<br>("+
            f1.auto_initial_cost + "&pound;-"+
            f1.auto_final_cost + "&pound;)/"+
    ...
    {
        case "semestral":
            insurance_text = f1.insurance_value + " Pounds per semester";
            break;
        case "anual":
            insurance_text = f1.insurance_value + " Pounds per year";
            break;
        case "mensal":
            insurance_text = data.monthly_costs.insurance + " Pounds per month";
            break;
        case "trimestral":
            insurance_text = f1.insurance_value + " Pounds per trimester";
            break;
    }
    ...
    if(f1.cred_auto_s_n == "true") {
        ...
        interests_text = "<b>Loan interests</b>&nbsp;&nbsp;<br>Financed amount: " +
                         f1.credit_amount +
                         "&pound;<br>Credit period / number of credit instalments: " +
                         f1.credit_period +
                         " months<br>Monthly average value: " +
                         f1.credit_value_p_month +
                         "&pound;<br>Residual value: " +
                         f1.credit_residual_value +
                         "&pound;<br>";    

        interests_text += "Total amount of interests: "+data.total_interests+"&pound;<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Monthly amount on interests: "+data.monthly_costs.credit.toFixed(2)+"&pound;";
        interests_text += "";
    } 
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Vehicle inspection (MOT test)</b><br>" +
                          f1.nmr_times_inspec +
                          " times costing " +
                          f1.inspec_price +
                          " &pound; each one during " +
                          data.age_months + " months&nbsp;";
    }
    else        
        inspection_text = "<b>Vehicle inspection (MOT test)</b><br>";
    ...
    //Taxes
    var cartax_text = "<b>Vehicle Excise Duty (Car tax)</b><br>" +
                      f1.car_tax + " Pounds per year";
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                fuel_text = fuel_text + "<br>" + "Fuel efficiency of your vehicle: " + f2.car_consumption + " mpg(imp)&nbsp;";
                fuel_text = fuel_text + "<br>" + "Fuel average price: " + f2.fuel_price + " &pound;/ltr&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " Day(s) per week you drive to work <br>";
                fuel_text = fuel_text + "You drive " + f2.distance_home2job + " miles between home and workplace <br>";
                fuel_text = fuel_text + "You drive " + f2.distance_weekend + " miles on average during the days you don't take your car to workplace&nbsp;<br>";
                fuel_text = fuel_text + "You drive totally on average of " + data.distance_per_month.toFixed(1) + " mi per month (~30.5 days) <br>";
                fuel_text = fuel_text + "Fuel efficiency of your vehicle: " + f2.car_consumption + " mpg(imp)";
                fuel_text = fuel_text + "<br>" + "Average price you pay for the fuel: " + f2.fuel_price + " &pound;/ltr";
            }
            break;
        case "euros":
            ...
            {
                case "1":
                    fuel_text = f2.fuel_money + " Pounds per month";
                    break;
                case "2":                   
                    fuel_text = f2.fuel_money + " miles for every two months";
                    break;
                case "3":                   
                    fuel_text = f2.fuel_money + " Pounds per trimester";
                    break;
                case "4":                   
                    fuel_text = f2.fuel_money + " Pounds per semester";
                    break;
                case "5":                   
                    fuel_text = f2.fuel_money + " Pounds per year";
                    break;
            }
            break;
    }
    ...
    var maintenance_text = "<b>1/2 Maintenance</b><br>" +
                           f2.maintenance + " Pounds per year";
    
    //Repairs
    var repairs_text = "<b>Repairs and improvements<\/span></b><br>" +
                       f2.repairs + " Pounds per year";
    ...
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                tolls_text += f2.tolls + " Pounds per month";
                break;
            case "2":
                tolls_text += f2.tolls + " Pounds per each two months";
                break;
            case "3":
                tolls_text += f2.tolls + " Pounds per trimester";
                break;
            case "4":
                tolls_text += f2.tolls + " Pounds per semester";
                break;
            case "5":
                tolls_text += f2.tolls + " Pounds per year";
                break;
        }
    }
    else 
        tolls_text+=f2.price_tolls_p_day + " Pounds during " + f2.tolls_days_p_month + " month";
    tolls_text += "";
    ...
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Pounds per month";
            break;
        case "2":           
            fines_text += f2.fines + " Pounds per each two months";
            break;
        case "3":           
            fines_text += f2.fines+" Pounds per trimester";
            break;
        case "4":           
            fines_text += f2.fines + " Pounds per semester";
            break;
        case "5":           
            fines_text += f2.fines + " Pounds per year";
            break;
        }
    fines_text+="";
    ...
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Pounds per month";
            break;
        case "2":
            washing_text += f2.washing + " Pounds per each two months";
            break;
        case "3":
            washing_text += f2.washing +" Pounds per trimester";
            break;
        case "4":
            washing_text += f2.washing + " Pounds per semester";
            break;
        case "5":
            washing_text += f2.washing + " Pounds per year";
            break;
        }
    washing_text+="";
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        taxi_text="<b>Taxi transportation<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " mi by taxi paying " + data.public_transports.taxi_price_per_km.toFixed(1) + "&pound;/mi";
        ...
        if (res_uber_obj){
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Public transports for your family daily life</b><br>Number of members of your family older than 4 years old: " + f3.n_pess_familia + " person(s)" +
                           "<br>Average amount of the monthly season ticket per person: " + f3.monthly_pass_cost + "&pound;</td>" +
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    var epa_text      = "<b>Emissões de poluentes atmosféricos</b><br>Valor aproximado: " + data.external_costs.polution + "&pound;/mi";
    var egee_text     = "<b>Emissões de gases de efeito de estufa</b><br>Valor aproximado: " + data.external_costs.ghg + "&pound;/mi";
    var ruido_text    = "<b>Poluição sonora</b><br>Valor aproximado: " + data.external_costs.noise + "&pound;/mi";
    var sr_text       = "<b>Sinistralidade rodoviária</b><br>Valor aproximado: " + data.external_costs.fatalities + "&pound;/mi";
    var cgstn_text    = "<b>Congestionamento<\/span></b><br>Valor aproximado: " + data.external_costs.congestion + "&pound;/mi";
    var ifr_estr_text = "<b>Desgaste das infraestruturas rodoviárias</b><br>Valor aproximado: " + data.external_costs.infrastr + "&pound;/mi";
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        varResult+="<tr><td>" + epa_text + "</td>" +   
                   "<td>&nbsp;&pound; " + (data.external_costs.polution * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + egee_text + "</td>" + 
                   "<td>&nbsp;&pound; " + (data.external_costs.ghg * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ruido_text + "</td>" + 
                   "<td>&nbsp;&pound; " + (data.external_costs.noise * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + sr_text + "</td>" + 
                   "<td>&nbsp;&pound; " + (data.external_costs.fatalities * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + cgstn_text + "</td>" + 
                   "<td>&nbsp;&pound; " + (data.external_costs.congestion * data.distance_per_month).toFixed(1)+"</td></tr>";
                
        varResult+="<tr><td>" + ifr_estr_text + "</td>" + 
                   "<td>&nbsp;&pound; " + (data.external_costs.infrastr * data.distance_per_month).toFixed(1)+"</td></tr>";
        
         //total
        varResult+="<tr><td style=\"padding:6px 10px 6px 0;\"><b>TOTAL</b></td>" +
                   "<td><b>&pound;&nbsp;"+data.external_costs.total_exter().toFixed(0)+"/month</b></td></tr>";
