...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocostos.info/MX',
            text: 'Esta calculadora te permitirá conocer el verdadero costo de ser propietario de un vehículo en México',
            showLabel: false,
            showCount: false,
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Verificación Vehicular</b><br>" +
                          f1.nmr_times_inspec +
                          " veces, costando " +
                          f1.inspec_price +
                          " &#36; cada una durante " +
    ...
    //Taxes
    var cartax_text = "<b>Tenencia</b><br>" +
...
function print_feffort_table(f1, f2, f3, data){
    ...
    if(f3.drive_to_work == 'true' || f2.take_car_to_job == 'true'){
        varResult+= "<tr><td>Cantidad de minutos que conduces para llegar de casa al trabajo</td>" + 
                    ...
                    "<tr><td>Cantidad de minutos que conduces por semana</td>" + 
                    "<td>" + data.time_spent_driving.min_drive_per_week + " min</td></tr>";
    }
    else{
        varResult+= "<tr><td>Cantidad de minutos que conduces por día</td>" + 
                    "<td>" + f3.min_drive_per_day + " min</td></tr>" +
                    "<tr><td>Cantidad de días mensuales que conduces</td>" + 
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        if(data.public_transports.pt_carcost_ratio < data.public_transports.other_pt_ratio_threshold){
            outros_tp_text="<b>Otros transportes públicos</b><br>Monto que pagaste por otro tipo de transporte público fuera de tu zona, como viajes en tren y autobuses de larga distancia ";
        }
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>Cantidad de minutos que conduces por mes</b></td>" + 
                ...
                varResult+="<tr><td><b>Otros transportes públicos</b><br>Monto que pagaste por otro tipo de transporte público fuera de tu zona, como viajes en tren y autobuses de larga distancia</td>" + 
