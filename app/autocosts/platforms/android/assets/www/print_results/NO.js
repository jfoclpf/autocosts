...
function Run2(callback){
    ...
    if(SOCIAL_SWITCH){
        $("#shareIcons").jsSocials({
            url: 'http://autocosts.info/NO',
            text: 'Denne kalkulatoren lar deg regne ut hva den virkelige prisen på å eie en bil i Norge er',
            showLabel: false,
            showCount: false,
...
function print_main_table(f1, f2, f3, data) {
    ...
    //header
    varResult+= '<tr><td style="padding:7px;" colspan="4"><b>BILEN DIN KOSTER</b></td></tr>';
    ...
    if(f3.IsFinancialEffort){
        varResult+= '<tr><td colspan="4"><b>ØKONOMISK INNSATS'+
...
function print_result_final_text(data){
    ...
    if(data.total_costs_month >= 150 && data.age_months > 6) {
        ...
        var text_msg = '<div>Med dette kostnadsnivået har bilen i løpet av det '+
                       '<b>' + data.age_months + '</b> måneder lange eierskapet allerede kostet:</div>'+
...
function print_costs_table(f1, f2, f3, data) {
    ...
    if (data.age_months === 0) {    
        depreciation_text = "Nedbetaling er ikke tellende ettersom bilen er ny&nbsp;&nbsp;";
    } else {
        depreciation_text = "<b>Nedbetaling av kjøretøyet<\/span></b>&nbsp;&nbsp;<br>Anskaffelsesverdi: "+
            f1.auto_initial_cost + "kr<br>Total verdi: "+
            f1.auto_final_cost + "kr<br>Periode for eierskap: "+
    ...
    {
        case "semestral":
            insurance_text = f1.insurance_value + " Kroner per halvår";
            break;
        case "anual":
        ...
        case "mensal":
            ...
            break;
        case "trimestral":
            insurance_text = f1.insurance_value + " Kroner per kvartal";
            break;
    }
    
    //Credit interests
    var interests_text="<b>Låneinteresse<\/b>&nbsp;&nbsp;";
    
    if(f1.cred_auto_s_n == "true") {

        interests_text = "<b>Låneinteresse</b>&nbsp;&nbsp;<br>Lånesum: " +
                         f1.credit_amount +
                         "kr<br>Kredittperiode / Antall avdrag: " +
                         f1.credit_period +
                         " måneder<br>Månedlig gjennomsnittsverdi: " +
                         f1.credit_value_p_month +
                         "kr<br>Restverdi: " +
                         f1.credit_residual_value +
                         "kr<br>";    

        interests_text += "Totalt interessebeløp: "+data.total_interests+"kr<br>(" + data.month_cred + "*"+ f1.credit_value_p_month + ")+" + f1.credit_residual_value + "-" + f1.credit_amount;

        if(data.age_months >= data.month_cred)
            interests_text += "<br>Månedlig interessebeløp: "+data.monthly_costs.credit.toFixed(2)+"kr";
        interests_text += "";
    } 
    ...
    if (f1.nmr_times_inspec !== 0){
        inspection_text = "<b>Bilinspeksjon</b><br>" +
                          f1.nmr_times_inspec +
                          " ganger som kostet " +
                          f1.inspec_price +
                          " kr per sjekk i løpet av " +
                          data.age_months + " måneder&nbsp;";
    }
    else        
        inspection_text = "<b>Bilinspeksjon</b><br>";
    ...
    //Taxes
    var cartax_text = "<b>Bilskatt for bilen</b><br>" +
    ...
    switch(f2.type_calc_fuel){
        case "km":                     
            if (f2.take_car_to_job == "false"){
                ...
                {
                    case "1":
                        fuel_text = f2.distance + " mil per måned";
                        break;
                    case "2":                   
                        fuel_text = f2.distance + " mil for annenhver måned";
                        break;
                    case "3":                   
                        fuel_text = f2.distance + " mil per kvartal";
                        break;
                    case "4":                   
                        fuel_text = f2.distance + " mil per halvår";
                        break;
                    case "5":                   
                        fuel_text = f2.distance + " mil per år";
                        break;
                }
                fuel_text = fuel_text + "<br>" + "Bilens drivstoffeffektivitet: " + f2.car_consumption + " l/10km&nbsp;";
                fuel_text = fuel_text + "<br>" + "Gjennomsnittlig drivstoffpris: " + f2.fuel_price + " kr/l&nbsp;&nbsp;";
            }
            else{
                fuel_text = f2.days_p_week + " Dag(er) i uken du kjører til jobb <br>";
                fuel_text = fuel_text + "Du kjører " + f2.distance_home2job + " mil mellom hjem og arbeidsplass <br>";
                fuel_text = fuel_text + "Du kjører " + f2.distance_weekend + " Mil i gjennomsnitt i løpet av dagene du ikke kjører til arbeidsplassen &nbsp;<br>";
                fuel_text = fuel_text + "Du kjører i gjennomsnitt totalt " + data.distance_per_month.toFixed(1) + " mil per måned (~30.5 dager) <br>";
                fuel_text = fuel_text + "Bilens drivstoffeffektivitet: " + f2.car_consumption + " l/10km";
                fuel_text = fuel_text + "<br>" + "Gjennomsnittlig pris for drivstofforbruket: " + f2.fuel_price + " kr/l";
            }
            break;
        case "euros":
            switch(data.fuel_cost_period)
            {
                case "1":
                    ...
                    break;
                case "2":                   
                    fuel_text = f2.fuel_money + " mil for annenhver måned";
                    break;
                case "3":                   
                    fuel_text = f2.fuel_money + " Kroner per kvartal";
                    break;
                case "4":                   
                    fuel_text = f2.fuel_money + " Kroner per halvår";
                    break;
                case "5":                   
    ...
    //Maintenance
    var maintenance_text = "<b>1/2 Vedlikehold</b><br>" +
                           f2.maintenance + " Kroner per år";
    
    //Repairs
    var repairs_text = "<b>Reparasjoner og forbedringer<\/span></b><br>" +
                       f2.repairs + " Kroner per år";
    
    //Tolls
    var tolls_text="<b>Bompenger</b><br>";
    if(f2.type_calc_tolls == "false") {
        switch(data.tolls_period) {
            case "1":
                ...
                break;
            case "2":
                tolls_text += f2.tolls + " Kroner per hver to måned";
                break;
            case "3":
                tolls_text += f2.tolls + " Kroner per kvartal";
                break;
            case "4":
                tolls_text += f2.tolls + " Kroner per halvår";
                break;
            case "5":
        ...
        tolls_text+=f2.price_tolls_p_day + " Kroner under " + f2.tolls_days_p_month + " måned";
    tolls_text += "";
    
    //Fines
    var fines_text="<b>Trafikkbøter</b><br>";
    switch(data.fines_period) {
        case "1":           
            fines_text += f2.fines + " Kroner per måned";
            break;
        case "2":           
            fines_text += f2.fines + " Kroner per hver to måned";
            break;
        case "3":           
            fines_text += f2.fines+" Kroner per kvartal";
            break;
        case "4":           
            fines_text += f2.fines + " Kroner per halvår";
            break;
        case "5":           
    ...
    //washing
    var washing_text="<b>Bilvask</b><br>";
    switch(data.washing_period) {
        case "1":
            washing_text += f2.washing + " Kroner per måned";
            break;
        case "2":
            washing_text += f2.washing + " Kroner per hver to måned";
            break;
        case "3":
            washing_text += f2.washing +" Kroner per kvartal";
            break;
        case "4":
            washing_text += f2.washing + " Kroner per halvår";
            break;
        case "5":
    ...
    varResult+= "<tr><td style=\"padding:10px 50px;\" colspan=\"2\"><b>Faste kostnader</b><br>" +
                "<i>Kostnader som ikke avhenger av kjørelengden og som må betales selv om bilen alltid står i ro</i></td></tr>";
    
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Kostnad</b></td>" +
                "<td><b>Månedlig beløp</b></td></tr>";
    
    //standing costs items
    varResult+= "<tr><td>" + depreciation_text + "&nbsp;</td>" + 
                "<td>&nbsp;" + currencyShow(data.monthly_costs.depreciation.toFixed(1)) + "</td></tr>";
    
    varResult+= "<tr><td><b>Bilforsikring og veihjelp</b><br>" + insurance_text + "</td>" +
    ...
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>TOTALT - Faste kostnader</b></td>"+
    ...
    varResult+= "<tr><td style=\"padding:10px 15px;\" colspan=\"2\"><b>Påløpende kostnader</b><br>" +
                "<i>Kostnader som avhenger av kjørelengde</i></td></tr>";
                  
    //Costs || Monthly amount
    varResult+= "<tr><td style=\"padding:10px 15px 10px 15px;\"><b>Kostnad</b></td>" +
                "<td><b>Månedlig beløp</b></td></tr>";

    varResult+= "<tr><td><b>Drivstoff</b><br>" + fuel_text + "</td>" +
    ...
    //TOTAL - Running costs
    varResult+= "<tr><td style=\"padding:4px 10px 4px 0;\"><b>TOTALT - Påløpende  kostnader</b></td>"+
    ...
    if((typeof data.distance_per_month) !== 'undefined' && data.distance_per_month !== 0){
        ...
        varResult+= "<tr><td><b>Løpende kostnader per kilometer</b></td>"+
    ...
    varResult+= "<tr><td><b>Faste kostnader</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_standing_costs_month.toFixed(0)) + "/måned</b></td></tr>";
    
    varResult+= "<tr><td><b>Påløpende kostnader</b></td>"+
                "<td>&nbsp;<b>" + currencyShow(data.total_running_costs_month.toFixed(0)) + "/måned</b></td></tr>";    
...
function print_feffort_table(f1, f2, f3, data){
    ...
    varResult+="<tr><td colspan=\"2\"><b>Økonomisk innsats</b></td></tr>";
    //income
    varResult+="<tr><td colspan=\"2\"><b>Income</b></tr>";
    switch(f3.income_type){
        case 'year':    
            varResult+= "<tr><td>Netto inntekt per år</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Gjennomsnittlig netto inntekt per måned</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>";
            break;
        case 'month':
            varResult+= "<tr><td>Netto inntekt per måned</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>" +
                        "<tr><td>Måneder i året med inntekt</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Gjennomsnittlig netto inntekt per måned</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>" +
                        "<tr><td>Gjennomsnittlig netto inntekt per år</td>" + 
                        "<td>" + currencyShow(data.fin_effort.income_per_year.toFixed(1)) + "</td></tr>";
            break;
        case 'week':
            varResult+= "<tr><td>Netto inntekt per uke</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Uker i året med inntekt</td>" + 
                        "<td>" + data.fin_effort.income_per_type + "</td></tr>" +
                        "<tr><td>Gjennomsnittlig netto inntekt per måned</td>" + 
                        "<td>" + currencyShow(data.fin_effort.aver_income_per_month.toFixed(1)) + "</td></tr>"+
                        "<tr><td>Gjennomsnittlig netto inntekt per år</td>" + 
            ...
            break;  
        case 'hour':
            varResult+= "<tr><td>Netto inntekt per time</td>" + 
                        "<td style=\"width:20%\">" + currencyShow(data.fin_effort.income) + "</td></tr>"+
                        "<tr><td>Timer i uken med inntekt</td>" + 
    ...
    if(f3.income_type != 'hour'){
        varResult+=     "<tr><td colspan=\"2\"><b>Arbeidstid</b></tr>";
        if(f3.is_working_time == 'true'){
            varResult+= "<tr><td>Timer per uke</td>" +
                        "<td>"+data.fin_effort.time_hours_per_week+" t</td></tr>" +
                        "<tr><td>Måneder per år</td>" + 
                        "<td>"+data.fin_effort.time_month_per_year+"</td></tr>" +
                        "<tr><td>Gjennomsnittlig arbeidstimer per måned</td>" + 
                        "<td>"+data.fin_effort.aver_work_time_per_m.toFixed(1)+" t</td></tr>" +
                        "<tr><td>Arbeidstimer per år</td>" + 
                        "<td>"+data.fin_effort.work_hours_per_y.toFixed(1)+" t</td></tr>";
        }
        else{
            varResult+= "<tr><td colspan=\"2\">Beregningene tar utgangspunkt for gjennomsnittlig arbeidstid på 36 timer i uken og 11 måneder i året</td></tr>";
        }
    }           
    varResult+= "<tr><td>Gjennomsnittlig netto inntekt per time</td>" +
                "<td>&nbsp;" + currencyShow(data.fin_effort.aver_income_per_hour.toFixed(1)) + "</td></tr>";
    
    //distance
    varResult+= "<tr><td colspan=\"2\"><b>Avstand</b></td></tr>";
    if((f2.type_calc_fuel != 'km' && f3.drive_to_work == 'true') || (f2.type_calc_fuel != 'km' && f2.take_car_to_job == 'true')){   
        varResult+=  "<tr><td>Du kjører til jobb fra hjemme</td>"  +  
                     "<td>" + parseInt(f3.dist_home_job).toFixed(1) + " mil</td></tr>"+
                     "<tr><td>Dager i uken du kjører til jobb</td>"  +  
                     "<td>" + f3.drive_to_work_days_per_week + " dager</td></tr>" +
                     "<tr><td>Du kjører de dagene du ikke kjører til jobb</td>" + 
                     "<td>" + parseInt(f3.journey_weekend).toFixed(1) + " mil</td></tr>"+
                     "<tr><td>Du kjører i gjennenomsnitt per uke</td>" + 
                     "<td>" + data.driving_distance.aver_drive_per_week.toFixed(1) + " mil</td></tr>";                  
    }

    varResult+=  "<tr><td>Du kjører per måned</td>" +
                 "<td>" + data.distance_per_month.toFixed(1) + " mil</td></tr>" +
                 "<tr><td>Du kjører per år</td>" + 
                 "<td>" + data.driving_distance.drive_per_year.toFixed(1) + " mil</td></tr>";  

    //time spent in driving
    varResult+=  "<tr><td colspan=\"2\"><b>Tid brukt på kjøring</b></td></tr>";

    if(f3.drive_to_work == 'true' || f2.take_car_to_job == 'true'){
        varResult+= "<tr><td>Minutter du kjører fra hjem til arbeidsplass</td>" + 
                    "<td>" + f3.time_home_job + " min</td></tr>" +
                    "<tr><td>Dager i uken du kjører til jobb</td>" + 
                    "<td>" + f3.drive_to_work_days_per_week + " dager</td></tr>" +
                    "<tr><td>Minutter du kjører de dagene du ikke kjører til jobb</td>" + 
                    "<td>" + f3.time_weekend + " min</td></tr>" +
                    "<tr><td>Minutter du kjøer per uke</td>" + 
                    "<td>" + data.time_spent_driving.min_drive_per_week + " min</td></tr>";
    }
    else{
        varResult+= "<tr><td>Minutter du kjøer per dag</td>" + 
                    "<td>" + f3.min_drive_per_day + " min</td></tr>" +
                    "<tr><td>Dager du kjører per måned</td>" + 
                    "<td>" + f3.days_drive_per_month + " dager</td></tr>";
    }

    varResult+= "<tr><td>Timer du kjører per måned</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_month.toFixed(1) + " t</td></tr>"+
                "<tr><td>Timer du kjører per år</td>" + 
                "<td>" + data.time_spent_driving.hours_drive_per_year.toFixed(1) + " t</td></tr>";

    //financial effort
    varResult+= "<tr><td colspan=\"2\"><b>Økonomisk innsats" +
                ': ' + (data.total_costs_year/data.fin_effort.income_per_year*100).toFixed(0) + 
                '&#37;</b>' +
                "<tr><td>Total kostnad per år for bil</td>" + 
                "<td>" + currencyShow(data.fin_effort.total_costs_year.toFixed(1)) + "</td></tr>" +
                "<tr><td>Timer per år du trenger å jobbe for å ha råd til bilen</td>"  +  
                "<td>" + data.fin_effort.hours_per_year_to_afford_car.toFixed(1) + " t</td></tr>"+
                "<tr><td>Hvor mange måneder per år du trenger å jobbe for å ha råd til bilen</td>" +  
                "<td>" + data.fin_effort.month_per_year_to_afford_car.toFixed(2)+"</td></tr>"+
                "<tr><td>Hvor mange dager, etter 1. januar, biler er betalt</td>" +  
                "<td>" + Math.ceil(data.fin_effort.days_car_paid) + " dager</td></tr>";
           

    //speed
    varResult+= "<tr><td>Årlig gjennomsnitt kinetisk hastighet</td>"+
                "<td>" + data.kinetic_speed.toFixed(1) + " mil/h</td></tr>";
                        
    varResult+= "<tr><td>Årlig gjennomsnitt <a href=\"./docs/consumer_speed.html\" target=\"_blank\">virtuell hastighet</a></td>"+
...
function print_AlternativeToCarCosts_table(f1, f2, f3, data, country){
    ...
    if(data.public_transports.display_pt()) {
        ...
        tp_text = "<b>Offentlig transport for familiens dagligliv</b><br>Antall familiemedlemmer som er eldre enn 4 år: " + 
                  f3.n_pess_familia + " person(er)" +
                  "<br>Gjennomsnittlig pris på månedskort per person: " + 
                  f3.monthly_pass_cost + "kr";
        
        if(data.public_transports.pt_carcost_ratio < data.public_transports.other_pt_ratio_threshold){
            outros_tp_text="<b>Annen offentlig transport</b><br>Beløp som var igjen til annen offentlig transport, for eksempel tog eller buss ";
        }
        taxi_text="<b>Taxi-transport<\/span><\/b><br>" + data.public_transports.km_by_taxi.toFixed(1) + " mil med taxi, betaler " + data.public_transports.taxi_price_per_km.toFixed(1) + "kr/mil";
    ...
    if(UBER_SWITCH){
        ...
        if (res_uber_obj){
            ...
            if(res_uber_obj.result_type==1){
                ...
                varResult+="<tr><td><b>UBER - Kostnad per minutter</b>" + uber_url_HTML + "</td>" + 
                           "<td>" + currencyShow(res_uber_obj.ucm.toFixed(2)) + "/" + "min</td></tr>";

                varResult+="<tr><td><b>Mil du kjører per måned</b><br></td>"+
                           "<td>" + res_uber_obj.dpm.toFixed(0) + " " +"mil</td></tr>";
                
                           
                varResult+="<tr><td><b>Minutter du kjøer per måned</b></td>" + 
                           "<td>" + res_uber_obj.mdpm.toFixed(0) + " " + "minutter</td></tr>";
                           
                varResult+="<tr><td><b>UBER: Kostnad - TOTALT</b></td>" + 
                           "<td><b>" + currencyShow(res_uber_obj.tuc.toFixed(0)) + "</b></td></tr>";                     

                varResult+="<tr><td><b>Annen offentlig transport</b><br>Beløp som var igjen til annen offentlig transport, for eksempel tog eller buss</td>" + 
            ...
            //the case where uber equivalent is more expensive
                ...
                varResult+="<tr><td><b>Offentlig transport for familiens dagligliv</b><br>Antall familiemedlemmer som er eldre enn 4 år: " + f3.n_pess_familia + " person(er)" +
                           "<br>Gjennomsnittlig pris på månedskort per person: " + f3.monthly_pass_cost + "kr</td>" +
                ...
                varResult+="<tr><td><b>UBER - mil per måned</b></td>" + 
...
function print_extern_table(f1, f2, f3, data){ 
    ...
    if(Country=="PT" && isDef(data.distance_per_month)){
        ...
        //header
        varResult+="<tr><td><b>Custos externos</b><br>Percorre " +(1 * data.distance_per_month).toFixed(1)+" mil/måned</td>" +
                   "<td><b>Månedlig beløp</b></td></tr>";
