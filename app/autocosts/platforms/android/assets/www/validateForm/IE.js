...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.IUC.value)) {
        $.jAlert({
            'title': "Vehicle Excise Duty (Motor tax)",
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Fuel",
            'content': "You must refer if you want to make calculations based on pounds or on kilometres!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.km_por_mes.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid amount of kilometres travelled per month!"
                });                
                return false;
            }
            ...
            if(!isNumber(f.km_entre_casa_trabalho.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid kilometres between home and workplace!"
                });                 
                return false;
            }
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid number of kilometres you do during the days you don't take your car to workplace!"
                });                 
                return false;
            }
