...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.seguro_val.value)){
        ...
        $.jAlert({ 
            'title': "Seguro de responsabilidad civil por daños a terceros",
    ...
    if(cred_auto_s_n == "true") {
        ...
        if(!isNumber(f.cred_auto_period.value)) {
            $.jAlert({
                'title': "Financiación del vehículo",
                'content': "Periodo de crédito o cantidad de cuotas no válido!"
            });             
            return false;
        }
    ...
    if(!isNumber(nmr_times_inspec) || !isInteger(nmr_times_inspec)) {
        $.jAlert({
            'title': "Inspección técnico-mecánica y de emisión de gases",
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Combustible",
            'content': "Debes indicar si prefieres hacer el cálculo basado en córdobas o kilómetros!"
        });        
        return false;
    }
    ...
    case "euros":/*fuel costs based on data input money per period of time*/
        ...
        if(!isNumber(f.combustiveis_euro.value)){
            $.jAlert({
                'title': "Combustible",
                'content': "Valor de córdobas por mes no válido!"
            });             
            return false;
        }
        break;
