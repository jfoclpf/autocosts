...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Combustible",
            'content': "Debes indicar si prefieres hacer el cálculo basado en quetzales o kilómetros!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if(leva_auto_job == ""){
            $.jAlert({
                'title': "Combustible",
                'content': "Indica, por favor, si utilizas el auto para ir al trabajo!"
            });            
            return false;
        }
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Cantidad de kilómetros recorridos los días que no vas en auto al trabajo no válida!"
                });                 
                return false;
            }
    ...
    case "euros":/*fuel costs based on data input money per period of time*/
        ...
        if(!isNumber(f.combustiveis_euro.value)){
            $.jAlert({
                'title': "Combustible",
                'content': "Valor de pesos por mes no válido!"
            });             
            return false;
        }
        break;
