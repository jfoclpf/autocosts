...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.seguro_val.value)){
        ...
        $.jAlert({ 
            'title': "Vehicle insurance and breakdown coverage",
    ...
    if(tipo_seguro_auto == ""){
        ...
        $.jAlert({
            'title': "Vehicle insurance and breakdown coverage",
            'content': "Insert payment frequency of insurance payments!"
        });        
        return false;
    }
    ...
    if(!isNumber(f.auto_val_final.value)){
        $.jAlert({
            'title': "Depreciation of the vehicle",
            'content': "Invalid present vehicle value!"
        });        
        return false;
    }
    ...
    if(cred_auto_s_n == ""){
        $.jAlert({
            'title': "Car finance",
            'content': "Please determine if you used car financing!"
        });                 
        return false;
    }
    ...
    if(cred_auto_s_n == "true") {
        ...
        if(!isNumber(f.cred_auto_period.value)) {
            $.jAlert({
                'title': "Car finance",
                'content': "Invalid period of credit, number of installment payments!"
            });             
            return false;
        }
        if(!isNumber(f.cred_auto_val_mes.value)) {
            $.jAlert({
                'title': "Car finance",
                'content': "Invalid installment amount!"
            });             
            return false;
        }
    ...
    if(!isNumber(f.IUC.value)) {
        $.jAlert({
            'title': "Vehicle Excise Tax (Auto taxes, if applicable)",
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Fuel",
            'content': "You must determine if you want to make calculations based on kilometres or AUD!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if(!isNumber(f.consumo_auto.value)){
            $.jAlert({
                'title': "Fuel",
                'content': "Invalid gas efficiency amount!"
            });            
            return false;
        }
        ...
        if(leva_auto_job == ""){
            $.jAlert({
                'title': "Fuel",
                'content': "Please indicate if you take your car to the workplace!"
            });            
            return false;
        }
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.km_por_mes.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid amount of kilometres traveled per month!"
                });                
                return false;
            }
            ...
            if(!isNumber(f.km_entre_casa_trabalho.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid kilometres between home and the workplace!"
                });                 
                return false;
            }
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid number of kilometres you drive during the days you don't drive your car to the workplace!"
                });                 
                return false;
            }
    ...
    if(tipo_calc_portagens=="false") {/*monthly basis*/
        ...
        if(!isNumber(f.preco_portagens_por_dia.value)) {
            $.jAlert({
                'title': "Tolls",
                'content': "Daily amount you spend on tolls - Invalid amount!"
            });            
            return false;
        }
