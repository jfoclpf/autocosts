...
function is_userdata_formpart1_ok(){
    ...
    if(cred_auto_s_n == ""){
        $.jAlert({
            ...
            'content': "Indica si has financiado tu coche!"
        });                 
        return false;
    }
    ...
    if(cred_auto_s_n == "true") {
        ...
        if(!isNumber(f.cred_auto_period.value)) {
            $.jAlert({
                'title': "Financiación del vehículo",
                'content': "Periodo del crédito o cantidad de cuotas no válidos!"
            });             
            return false;
        }
        if(!isNumber(f.cred_auto_val_mes.value)) {
            $.jAlert({
                'title': "Financiación del vehículo",
                'content': "Valor de cuota no válido!"
            });             
            return false;
        }
    ...
    if(public_transport || fin_effort){
        ...
        if (isVisible('.time_spent_part1_form3')){        
            ...
            if(!isNumber(f.time_weekend.value)){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Cantidad de minutos empleados en conducción los días que no vas en coche al trabajo no válida!"
                });                 
                return false;
            }
        }
            ...
            if(!isNumber(days_drive_per_month) || !isInteger(days_drive_per_month) || days_drive_per_month>31){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Cantidad de días por mes no válida!"
                });                 
                return false;
            }
        }
