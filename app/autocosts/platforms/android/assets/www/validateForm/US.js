...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Fuel",
            'content': "You must determine if you want to make calculations based on miles or USD!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.km_por_mes.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid amount of miles traveled per month!"
                });                
                return false;
            }
            ...
            if(!isNumber(f.km_entre_casa_trabalho.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid miles between home and the workplace!"
                });                 
                return false;
            }
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Fuel",
                    'content': "Invalid number of miles you drive during the days you don't drive your car to the workplace!"
                });                 
                return false;
            }
