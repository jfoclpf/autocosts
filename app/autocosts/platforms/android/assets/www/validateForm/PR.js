...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.seguro_val.value)){
        ...
        $.jAlert({ 
            'title': "Seguro de Responsabilidad obligatorio",
    ...
    if(!isNumber(nmr_times_inspec) || !isInteger(nmr_times_inspec)) {
        $.jAlert({
            'title': "Inspección Mecánida Vehicular (IMV)",
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Combustible",
            'content': "Debes indicar si prefieres hacer el cálculo basado en dólares o kilómetros!"
        });        
        return false;
    }
    ...
    case "euros":/*fuel costs based on data input money per period of time*/
        ...
        if(!isNumber(f.combustiveis_euro.value)){
            $.jAlert({
                'title': "Combustible",
                'content': "Valor de dólares por mes no válido!"
            });             
            return false;
        }
        break;
