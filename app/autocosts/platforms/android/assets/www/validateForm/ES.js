...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.auto_val_final.value)){
        $.jAlert({
            'title': "Depreciación del vehículo",
            'content': "El precio actual no válido!"
        });        
        return false;
    }
    ...
    if(cred_auto_s_n == ""){
        $.jAlert({
            ...
            'content': "Indica por favor si has financiado tu coche!"
        });                 
        return false;
    }
    ...
    if(cred_auto_s_n == "true") {
        ...
        if(!isNumber(f.cred_auto_period.value)) {
            $.jAlert({
                'title': "Financiación del coche",
                'content': "Periodo de crédito o número de plazos no válido!"
            });             
            return false;
        }
    ...
    if(!isNumber(nmr_times_inspec) || !isInteger(nmr_times_inspec)) {
        $.jAlert({
            'title': "Inspección técnica del vehículo (ITV)",
            'content': "Número de veces no válido!"
        });
        return false;
    }
    ...
    if(!isNumber(f.preco_inspecao.value) && nmr_times_inspec!=0) {
        $.jAlert({
            'title': "Inspección técnica del vehículo (ITV)",
            'content': "Coste de inspección no válido!"
        });        
        return false;
    }
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Combustible",
            'content': "Debes indicar si prefieres hacer el cálculo basado en euro o kilómetros!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if(leva_auto_job == ""){
            $.jAlert({
                'title': "Combustible",
                'content': "Indica, por favor, si utilizas el coche para ir al trabajo!"
            });            
            return false;
        }
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.dias_por_semana.value) || (f.dias_por_semana.value)>7){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Número de días por semana no válido!"
                });                
                return false;
            }
            if(!isNumber(f.km_entre_casa_trabalho.value)){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Cantidad de kilómetros entre la casa y el lugar del trabajo no válida!"
                });                 
                return false;
            }
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Cantidad de kilómetros recorridos los días que no vas en coche al trabajo no válida!"
                });                 
                return false;
            }
    ...
    case "euros":/*fuel costs based on data input money per period of time*/
        ...
        if(!isNumber(f.combustiveis_euro.value)){
            $.jAlert({
                'title': "Combustible",
                'content': "Valor de euros por mes no válido!"
            });             
            return false;
        }
        break;
    ...
    if(tipo_calc_portagens=="false") {/*monthly basis*/
        ...
        if(!isNumber(f.preco_portagens_por_dia.value)) {
            $.jAlert({
                'title': "Peaje",
                'content': "Promedio de gastos de peaje a diario - Cuota no válida!"
            });            
            return false;
        }
    ...
    if(public_transport){
        ...
        if(!isNumber(n_pess_familia) || !isInteger(n_pess_familia) || n_pess_familia<=0){
            $.jAlert({
                ...
                'content': "Número de miembros no válido!"
            });           
            return false;
        }
        ...
        if(!isNumber(pmpmpc) || pmpmpc<0){
            $.jAlert({
                'title': "Información adicional",
                'content': "Coste mensual de abono no válido!"
            });            
            return false;
        }
    }
    ...
    if(fin_effort){
        ...
        case 'month':
            ...
            if(!isNumber(f.income_months_per_year.value)){
                $.jAlert({
                    'title': "Ingresos",
                    'content': "Número de meses anuales no válido!"
                });                 
                return false;
            }			
            break;
        case 'week':
            ...
            if(!isNumber(f.income_weeks_per_year.value)){
                $.jAlert({
                    'title': "Ingresos",
                    'content': "Número de semanas anuales no válido!"
                });                 
                return false;
            }			
            break;
        ...
        if(is_working_time == 'true' && income_type!='hour'){
            if(!isNumber(f.time_hours_per_week.value)){
                $.jAlert({
                    ...
                    'content': "Número de horas semanales no válido!"
                });                 
                return false;
            }
        ...
        if (isVisible('.time_spent_part1_form3')){        
            if(!isNumber(f.time_home_job.value)){
                $.jAlert({
                    ...
                    'content': "Número de minutos empleados en conducción de casa al trabajo no válido!"
                });                 
                return false;
            }
            if(!isNumber(f.time_weekend.value)){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Número de minutos empleados en conducción los días que no vas en coche al trabajo no válido!"
                });                 
                return false;
            }
        }
            ...
            if(!isNumber(f.min_drive_per_day.value)){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Número de minutos empleados en conducción no válido!"
                });                 
                return false;
            }
            ...
            if(!isNumber(days_drive_per_month) || !isInteger(days_drive_per_month) || days_drive_per_month>31){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Número de días mensuales no válido!"
                });                 
                return false;
            }
        }
