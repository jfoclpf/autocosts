...
function is_userdata_formpart1_ok(){
    ...
    if(!isNumber(f.seguro_val.value)){
        ...
        $.jAlert({ 
            'title': "Seguro de vehículo y SOAP",
    ...
    if(tipo_seguro_auto == ""){
        ...
        $.jAlert({
            'title': "Seguro de vehículo y SOAP",
            'content': "Ingresa la periodicidad del seguro!"
        });        
        return false;
    }
    ...
    if(!isNumber(nmr_times_inspec) || !isInteger(nmr_times_inspec)) {
        $.jAlert({
            ...
            'content': "Cantidad de veces no válido!"
        });
        return false;
    }
...
function is_userdata_formpart2_ok(){
    ...
    if(tipo_calc_combustiveis == ""){
        $.jAlert({
            'title': "Combustible",
            'content': "Debes indicar si prefieres hacer el cálculo basado en pesos (moneda) o kilómetros!"
        });        
        return false;
    }
    ...
    case "km": /*fuel calculations made considering distance travelled by month*/
        ...
        if(leva_auto_job == ""){
            $.jAlert({
                'title': "Combustible",
                'content': "Indica, por favor, si utilizas el vehículo para ir al trabajo!"
            });            
            return false;
        }
        ...
        if (leva_auto_job=="false"){
            ...
            if(!isNumber(f.dias_por_semana.value) || (f.dias_por_semana.value)>7){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Cantidad de días por semana no válido!"
                });                
                return false;
            }
            ...
            if(!isNumber(f.km_fds.value)){
                $.jAlert({
                    'title': "Combustible",
                    'content': "Cantidad de kilómetros recorridos los días que no vas en vehículo al trabajo no válida!"
                });                 
                return false;
            }
    ...
    if(public_transport){
        ...
        if(!isNumber(n_pess_familia) || !isInteger(n_pess_familia) || n_pess_familia<=0){
            $.jAlert({
                ...
                'content': "Cantidad de miembros no válido!"
            });           
            return false;
        }
    ...
    if(public_transport || fin_effort){
        ...
        if (isVisible('.time_spent_part1_form3')){        
            ...
            if(!isNumber(f.time_weekend.value)){
                $.jAlert({
                    'title': "Tiempo empleado en la conducción",
                    'content': "Cantidad de minutos empleados en conducción los días que no vas en vehículo al trabajo no válida!"
                });                 
                return false;
            }
        }
